...
//import biomight.chemistry.nucleicacids.nucleotides.*;
...
public class Adenine extends Nucleotide {
	...
	public Adenine()
	{		
		// Create the Base Adenine
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, Constants.AdenineRef, null, null);
	}
	
	public Adenine(String parentID)
	{				
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
	...
	public Adenine(String parentID, BioMightPosition bioMightPosition)
	{
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, parentID, null, null);
	...
	public Adenine(String parentID, ArrayList<BioMightMethodView> bioMightMethods)
	{		
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
	...
	// the Adenines parent object and we do not have  to go to the database 
	// to get additional informaion
	public Adenine(int localVP, int localLOD, String parentID, ArrayList<BioMightMethodView> bioMightMethods)
	{				
		create(localVP, localLOD, parentID, null, bioMightMethods);	
	}
	
	public Adenine(int localVP, int localLOD, String parentID,  ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	...
	{
			this.setImage("images/Adenine.jpg");
			...
			if (localVP == Constants.VIEW_INTERNAL)			
			{
				...
				System.out.println("In Adenine Create() - ViewInternal - Gathered by parent: " + parentID);				
				
				// We already have the data for the current instance of Adenine,
				// Go get the details for the current Adenine is LOD is set
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the Adenine				
					System.out.println("Adenine Create - ViewInternal - MAG2X: " + parentID);				
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					
					// Create the Adenine components
					//carbon = new Carbon(localVP, localLOD, componentID, bioMightMethods);									
					//System.out.println("Adenine Instance created from SubComponents : " + parentID);
			...
			{	
				this.componentID = parentID;
				System.out.println("In Adenine Create() - ViewDetached - Gather now by parent: " + parentID);				
				
				
				// Generate the Adenine that the user desires if needed 
				...
				// a collection of Adenine
				try {
					// Get the information from the database via the Enterprise Bean		
					System.out.println("Getting Adenine -Detached - Transform: " + componentID + "   " +  parentID);
					InitialContext ctx = new InitialContext();
					BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
					bioMightTransforms = bioMightBean.getComponents(Constants.AdenineRef, parentID);   	
				}catch (Exception e) { 
					System.out.println("Exception Getting Components - Adenine");
					throw new ServerException("Remote Exception getComponents():", e); 	
				}
						
				// Run through the collection of nucleobases and build them into the model
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("Have Adenine NumTransforms: " + transforms.size());
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					System.out.println("Creating Adenine: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					this.componentID = bioMightTransform.getId();
					
				
					if (localLOD == Constants.MAG1X)
					{
						System.out.println("Adenine Detached - 1x View - parent : " + parentID + " lod: " + localLOD);
					...
					{
						// Go get the finer details of the Adenine				
						System.out.println("Adenine Detached - 2x View Creating SubComponents : " + parentID + " lod: " + localLOD);
			...
			{
				// This is when one is accessing a Adenine directly
				// Get the information from the database via the Enterprise Bean		
				try {
					// Get the information from the database via the Enterprise Bean		
					System.out.println("Adenine Create() -HawkEye - Gather data from : " + parentID);
					InitialContext ctx = new InitialContext();
					BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
					bioMightTransforms = bioMightBean.getComponent(parentID);
					System.out.println("Have Adenine Info from EJB");   	
				}catch (Exception e) { 
					System.out.println("Exception Getting Components - Adenine");
					throw new ServerException("Remote Exception getComponents():", e); 	
				}
					
				// Run through the collection of Adenines and build them into the model
				// In the default case, we get one instance of the Adenine for each eye
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("Adenine NumTransforms: " + transforms.size());
				for (int i=0; i<transforms.size(); i++)
				{
					// Get the information for the Adenine
					BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
					System.out.println("Creating Adenine: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					this.componentID = bioMightTransform.getId();
					...
					{
						// initialize the Properties
						System.out.println("Creating Adenine at MAG1X : " + parentID);
						initProperties();
					}
					if (localLOD == Constants.MAG2X)
					{
						// Go get the finer details of the Adenine				
						System.out.println("Creating Adenine at MAG2X : " + parentID);
						localVP = Constants.VIEW_HAWKEYE; 
						localLOD = Constants.MAG1X;
			...
			System.out.println("CreateAdenine Completed");
			...
			if (bioMightMethods != null){
				System.out.println("EXECUTING Adenine Methods: " + bioMightMethods.size());
		...
		private void initProperties() {
			...
			System.out.println("Adenine InitProperties: " + componentID);
		...
		{
			// Generate the Adenine		
			BioMightDNABeanLocal bioMightBean;
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Generating the Adenine - component: " + componentID + "  parent: " + parentID);
				InitialContext ctx = new InitialContext();
				...
				//int success = bioMightBean.generateAdenine("Adenine:00001", "Adenine", 
				//		"Adenine", componentID, parentID, currentPoints);			
	
				
				System.out.println("Created Adenine Info using EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Adenine");
				throw new ServerException("Remote Exception Adenine():", e); 	
			}
		}
		...
		public void executeMethods(ArrayList<String> methodParams) {
			...
			// associated methods
			System.out.println("Adenine Executing Methods");
		 ...
		 * GET X3D
		 * 
		 * This method will return the X3D for the Adenine.  It runs through each of its components and collects up their
		...
		public String getX3D(boolean snipet) {
			...
			"<head>\n" +
			 "<meta name='BioMightImage' content='Adenine.jpg'/>\n" +
			...
			System.out.println("In Adenine - Getting X3D - Using HardValue of MAG1X");
			lod = Constants.MAG1X;
			...
			{	
				...
				System.out.println("In Adenine - Getting X3D - View Internal");
				...
				// We do nada as the Adenine Data is retrieved in the collection object
			...
			{			
				...
				System.out.println("In Adenine - Getting X3D - View Detached");
				if (lod == Constants.MAG1X)		
				{
					ArrayList transforms = bioMightTransforms.getTransforms();
					for (int i=0; i<transforms.size(); i++)
					{
						...
						BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
						System.out.println("In Adenine - Creating Adenine Detached: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
						System.out.println("In Adenine - Creating Adenine at Position: " + 
						...
						if (lod == Constants.MAG1X)		
						{
							//System.out.println("Getting X3D for AdenineX: " + bioMightTransform.getTranslation().getXPos());
							//System.out.println("Getting X3D for AdenineY: " + bioMightTransform.getTranslation().getYPos());
							//System.out.println("Getting X3D for AdenineZ: " + bioMightTransform.getTranslation().getZPos());
			...
			else if (viewPerspective == Constants.VIEW_HAWKEYE) 
			{
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
					System.out.println("Creating Adenine: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					System.out.println("Creating Adenine at Position: " + 
							bioMightTransform.getTranslation().getXPos() + "  " +
							bioMightTransform.getTranslation().getYPos() + "  " +
							bioMightTransform.getTranslation().getZPos());
			...
			//System.out.println("Adenine X3D: " + body);		
		...
		public String getX3D(BioMightTransform bioMightTransform) 
		{
			...
			double angle=0;
			System.out.println("Adenine.getLocalX3DZ()  " + bioMightTransform.getComponentName());	
