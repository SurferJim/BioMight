...
* @author SurferJim
*
* Representation of Leucine
*
*************************************************************************/
public class Leucine  extends BioMightBase {
	...
	public Leucine()
	{
		...
		create(localVP, localLOD, Constants.LeucineRef, null, null);
	}

	public Leucine(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Leucine(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Leucine.jpg");
		...
		System.out.println("Creating Leucine for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Leucine Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Leucine,
			// Go get the details for the current Leucine is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Leucine				
				System.out.println("Getting the Leucine MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;		
			
				System.out.println("Creating Leucine MAG ViewInternal - 2X : " + parentID);
				...
				System.out.println("In Leucine - CarbonsRef is complete");
				...
				//System.out.println("In Leucine - Hydrogens is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Leucine directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye LeucineInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Leucine Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Leucine");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Leucine for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Leucine NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Leucine at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Leucine				
					System.out.println("Creating Leucine at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;

					System.out.println("Creating Leucine HawkEye - 2X : " + parentID);
					carbons = new Carbons(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);
					initProperty(Constants.CarbonsRef, Constants.Carbons, Constants.CarbonsRef, carbons.getComponentID());
					System.out.println("In Leucine - CarbonsRef is complete");
		...
		System.out.println("CreateLeucine Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Leucine Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Leucine");
		method.setMethodName("Penetrate Leucine");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Leucine : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Leucine:01")) {
				...
				//int success = bioMightBean.generateLeucine("Leucine:00001", "Leucine", 
				//		"Leucine", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Leucine:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateLeucine("Leucine:00160", "Leucine", 
				//	"Leucine", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Leucine NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Leucine");
			throw new ServerException("Remote Exception Leucine():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Leucine.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Leucine.jpg'/>\n" +
		...
		{	
			...
			System.out.println("Get X3D Leucine - View Internal");
			// We do nada as the Leucine Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Leucine - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Leucine: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Leucine at Position: " + 
		...
		{
			System.out.println("Get X3D Leucine - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Leucine - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += carbons.getX3D(true);	
			}
			else if (lod == Constants.MAG1X)	
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Leucine - View Hawkeye  1X");
				...
				//*********************************************************************************
				// Create the Luecine side chain - CH2 - CH - CH3 - CH3
				//*********************************************************************************
				...
				double[] coOrientation2 = {0.0, 90.0};
				...
				String[] coAtoms3 = {"Hydrogen", "Hydrogen", "Hydrogen"};
				double[] coOrientation3 = {0.0, 90.0, 180.0};
		...
		body+= "<Viewpoint DEF='Viewpoint_Leucine'\n" +
				 "description='Viewpoint_Leucine'\n" +
				 "jump='true'\n" +
				 "fieldOfView='0.785'\n" +
