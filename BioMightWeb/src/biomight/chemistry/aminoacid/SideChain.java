 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 ******************************************************************************/
public class SideChain extends BioMightBase {
	private BioMightPosition bioMightPosition;
	

	public SideChain()
	{		
		// Create the Base SideChain
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, Constants.SideChainRef, null, null);
	}
	
	public SideChain(String parentID)
	{				
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
	...
	public SideChain(String parentID, BioMightPosition bioMightPosition)
	{
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, parentID, null, null);
	...
	public SideChain(String parentID, ArrayList<BioMightMethodView> bioMightMethods)
	{		
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
	...
	// the SideChains parent object and we do not have  to go to the database 
	// to get additional informaion
	
	public SideChain(int localVP, int localLOD, String parentID,  ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		System.out.println("In SideChain Calling Create()");				
	...
	{
			this.setImage("images/SideChain.jpg");
			...
			if (localVP == Constants.VIEW_INTERNAL)			
			{
				...
				System.out.println("In SideChain Create() - ViewInternal - Gathered by parent: " + parentID);				
			
				// We already have the data for the current instance of SideChain,
				// Go get the details for the current SideChain is LOD is set
				if (localLOD == Constants.MAG2X)
				{
					// Generate the SideChain that the user desires if needed 
					boolean bGenerate = false;
					if (bGenerate) {
						generate(parentID, componentID);
					}
					
					// Go get the finer details of the SideChain				
					System.out.println("Getting the SideChain Details: " + parentID);
			...
			{
				// This is when one is accessing a SideChain directly
				//System.out.println("In SideChain Create() - Getting Name from properties: " + bioMightProperties.get(0).getPropertyName());				
						
				// Get the information from the database via the Enterprise Bean		
				try {
					// Get the information from the database via the Enterprise Bean		
					System.out.println("Getting HawkEye SideChainInfo for ParentID: " + parentID);
					InitialContext ctx = new InitialContext();
					BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
					bioMightTransforms = bioMightBean.getComponent(parentID);
					System.out.println("Have SideChain Info from EJB");   	
				}catch (Exception e) { 
					System.out.println("Exception Getting Components - SideChain");
					throw new ServerException("Remote Exception getComponents():", e); 	
				}
					
				// Run through the collection of SideChains and build them into the model
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("SideChain NumTransforms: " + transforms.size());
				for (int i=0; i<transforms.size(); i++)
				{
					// Get the information for the SideChain
					BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
					System.out.println("Creating HawkEye SideChain: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					this.componentID = bioMightTransform.getId();
					String sideChain = bioMightTransform.getName();

					if (localLOD == Constants.MAG1X)
					{
						System.out.println("In SideChain Create() - ViewHawkEye - Just Gathered - parent: " + parentID);				
						// initialize the Properties
						initProperties(sideChain);
					}
					if (localLOD == Constants.MAG2X)
					{
						...
						System.out.println("SideChain Hawkeye - 2x View - parent: " + parentID + " lod: " + localLOD);
			...
			System.out.println("CreateSideChain Completed");
			...
			if (bioMightMethods != null){
				//System.out.println("EXECUTING SideChain Methods: " + bioMightMethods.size());
		...
		public void generate(String parentID, String componentID)
		{
			...
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Generating the SideChain : " + componentID + "    " + parentID);
				InitialContext ctx = new InitialContext();
				...
				// We should create the SideChain based upon where it is in the body.
	    		...
	    		// Does this deliver 1 SideChain record in the database or does it
	    		// create the subComponents?   - both is always the answer
				//int success = bioMightBean.generateSideChain(Constants.AdenineRef, "SideChain:00001", "SideChain", 
				//		"SideChain", componentID, parentID, currentPoints);			
					
				System.out.println("Created SideChain Info using EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - SideChain");
				throw new ServerException("Remote Exception SideChain():", e); 	
			}
		}
		...
		public void executeMethods(ArrayList<String> methodParams) {
			...
			// associated methods
			System.out.println("SideChain Executing Methods");
		 ...
		 * GET X3D
		 * 
		 * This method will return the X3D for the SideChain.  It runs through each of its components and collects up their
		 * representations and then assembles them into one unified X3D model.
		 * 
		 ********************************************************************************************************************/
		public String getX3D(boolean snipet) {
			
			// Assembe the SideChain
			String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
			"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
			"<X3D profile='Immersive' >\n" +
			"<head>\n" +
			 "<meta name='BioMightImage' content='SideChain.jpg'/>\n" +
			...
			{			
				...
				// We do nada as the SideChain Data is retrieved in the collection object
			...
			else if (viewPerspective == Constants.VIEW_HAWKEYE) 
			{
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
					System.out.println("In SideChain - Creating SideChain Hawkeye: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					System.out.println("In SideChain - Creating SideChain at Position: " + 
					...
					if (lod == Constants.MAG1X)		
					{
						//System.out.println("Getting X3D for SideChainX: " + bioMightTransform.getTranslation().getXPos());
						//System.out.println("Getting X3D for SideChainY: " + bioMightTransform.getTranslation().getYPos());
						//System.out.println("Getting X3D for SideChainZ: " + bioMightTransform.getTranslation().getZPos());
					...
					{
						System.out.println("In SideChain - Creating SideChain MAG2X ");
						//body +=  sideChain.getX3D(true);
					}
				}
			}
			else 
			...
			body+= "<Viewpoint DEF='Viewpoint_SideChain'\n" +
			...
			//System.out.println("SideChain X3D: " + body);		
		 ...
		 * GET LOCAL X3D
		 * 
		 * This method will return the X3D for the SideChains.  Each SideChain record from the database will yield
		...
		{
			angle=0;
			System.out.println("SideChain.getLocalX3DZ()  " + bioMightTransform.getComponentName());	
			...
			if (bioMightTransform.getComponentName().equals("Alanine"))
			{
				
				String componentTypeOut = "Adenine";
				body += "<Transform onmouseover=\"showComponent('Adenine SideChain');\" DEF='" + componentTypeOut + "'>\n";
			...
			else if (bioMightTransform.getComponentName().equals("Guanine"))
			{
				...
				body += "<Transform onmouseover=\"showComponent('Guanine SideChain');\" DEF='" + componentTypeOut + "'\n";
			...
			else if (bioMightTransform.getComponentName().equals("Cytosine"))
			{
				...
				body += "<Transform onmouseover=\"showComponent('Cytosine SideChain');\" DEF='" + componentTypeOut + "'\n";
			...
			else if (bioMightTransform.getComponentName().equals("Thymine"))
			{
				...
				body += "<Transform onmouseover=\"showComponent('Thymine SideChain');\" DEF='" + componentTypeOut + "'\n";
