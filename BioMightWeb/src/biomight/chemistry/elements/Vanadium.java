...
* @author SurferJim
*
* Representation of Vanadium
*
*************************************************************************/
public class Vanadium  extends BioMightBase {
	...
	public Vanadium()
	{
		...
		create(localVP, localLOD, Constants.VanadiumRef, null, null);
	}

	public Vanadium(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Vanadium(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Vanadium.jpg");
		...
		System.out.println("Creating Vanadium for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Vanadium Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Vanadium,
			// Go get the details for the current Vanadium is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Vanadium				
				System.out.println("Getting the Vanadium MAG2X : " + parentID);
				...
				//System.out.println("In Vanadium - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Vanadium directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye VanadiumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Vanadium Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Vanadium");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Vanadium for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Vanadium NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Vanadium at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Vanadium				
					System.out.println("Creating Vanadium at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateVanadium Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Vanadium Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Vanadium");
		method.setMethodName("Penetrate Vanadium");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Vanadium : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Vanadium:01")) {
				...
				//int success = bioMightBean.generateVanadium("Vanadium:00001", "Vanadium", 
				//		"Vanadium", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Vanadium:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateVanadium("Vanadium:00160", "Vanadium", 
				//	"Vanadium", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Vanadium NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Vanadium");
			throw new ServerException("Remote Exception Vanadium():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Vanadium.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the Vanadium
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='Vanadium.jpg'/>\n" +
		...
		{	
			...
			System.out.println("Get X3D Vanadium - View Internal");
			// We do nada as the Vanadium Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Vanadium - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Vanadium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Vanadium at Position: " + 
		...
		{
			System.out.println("Get X3D Vanadium - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Vanadium - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Vanadium - View Hawkeye  1X");
