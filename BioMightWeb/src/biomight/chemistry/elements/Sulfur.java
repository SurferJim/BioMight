...
* @author SurferJim
*
* Representation of Sulfur
*
*************************************************************************/
public class Sulfur  extends BioMightBase {
	...
	public Sulfur()
	{
		...
		create(localVP, localLOD, Constants.SulfurRef, null, null);
	}

	public Sulfur(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Sulfur(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Sulfur.jpg");
		...
		System.out.println("Creating Sulfur for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Sulfur Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Sulfur,
			// Go get the details for the current Sulfur is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Sulfur				
				System.out.println("Getting the Sulfur MAG2X : " + parentID);
				...
				//System.out.println("In Sulfur - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Sulfur directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye SulfurInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Sulfur Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Sulfur");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Sulfur for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Sulfur NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Sulfur at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Sulfur				
					System.out.println("Creating Sulfur at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateSulfur Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Sulfur Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Sulfur");
		method.setMethodName("Penetrate Sulfur");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Sulfur : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Sulfur:01")) {
				...
				//int success = bioMightBean.generateSulfur("Sulfur:00001", "Sulfur", 
				//		"Sulfur", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Sulfur:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateSulfur("Sulfur:00160", "Sulfur", 
				//	"Sulfur", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Sulfur NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Sulfur");
			throw new ServerException("Remote Exception Sulfur():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Sulfur.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Sulfur.jpg'/>\n" +
		...
		{	
			...
			System.out.println("Get X3D Sulfur - View Internal");
			// We do nada as the Sulfur Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Sulfur - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Sulfur: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Sulfur at Position: " + 
		...
		{
			System.out.println("Get X3D Sulfur - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Sulfur - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Sulfur - View Hawkeye  1X");
