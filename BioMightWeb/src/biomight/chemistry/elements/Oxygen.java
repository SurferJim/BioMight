...
*
*************************************************************************/
public class Oxygen  extends BioMightBase {
	...
	public Oxygen()
	{
		...
		create(localVP, localLOD, Constants.OxygenRef, null, null);
	}

	public Oxygen(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Oxygen(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Oxygen.jpg");
		...
		System.out.println("Creating Oxygen for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Oxygen Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Oxygen,
			// Go get the details for the current Oxygen is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Oxygen				
				System.out.println("Getting the Oxygen MAG2X : " + parentID);
				...
				//System.out.println("In Oxygen - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Oxygen directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye OxygenInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Oxygen Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Oxygen");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Oxygen for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Oxygen NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Oxygen at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Oxygen				
					System.out.println("Creating Oxygen at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateOxygen Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Oxygen Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Oxygen");
		method.setMethodName("Penetrate Oxygen");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Oxygen : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Oxygen:01")) {
				...
				//int success = bioMightBean.generateOxygen("Oxygen:00001", "Oxygen", 
				//		"Oxygen", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Oxygen:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateOxygen("Oxygen:00160", "Oxygen", 
				//	"Oxygen", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Oxygen NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Oxygen");
			throw new ServerException("Remote Exception Oxygen():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Oxygen.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Oxygen.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Oxygen!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Oxygen - View Internal");
			// We do nada as the Oxygen Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Oxygen - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Oxygen: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Oxygen at Position: " + 
		...
		{
			System.out.println("Get X3D Oxygen - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Oxygen - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Oxygen - View Hawkeye  1X");
