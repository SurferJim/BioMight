...
* @author SurferJim
*
* Representation of Selenium
*
*************************************************************************/
public class Selenium  extends BioMightBase {
	...
	public Selenium()
	{
		...
		create(localVP, localLOD, Constants.SeleniumRef, null, null);
	}

	public Selenium(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Selenium(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Selenium.jpg");
		...
		System.out.println("Creating Selenium for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Selenium Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Selenium,
			// Go get the details for the current Selenium is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Selenium				
				System.out.println("Getting the Selenium MAG2X : " + parentID);
				...
				//System.out.println("In Selenium - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Selenium directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye SeleniumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Selenium Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Selenium");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Selenium for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Selenium NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Selenium at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Selenium				
					System.out.println("Creating Selenium at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateSelenium Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Selenium Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Selenium");
		method.setMethodName("Penetrate Selenium");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Selenium : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Selenium:01")) {
				...
				//int success = bioMightBean.generateSelenium("Selenium:00001", "Selenium", 
				//		"Selenium", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Selenium:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateSelenium("Selenium:00160", "Selenium", 
				//	"Selenium", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Selenium NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Selenium");
			throw new ServerException("Remote Exception Selenium():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Selenium.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the Selenium
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='Selenium.jpg'/>\n" +
		...
		{	
			...
			System.out.println("Get X3D Selenium - View Internal");
			// We do nada as the Selenium Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Selenium - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Selenium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Selenium at Position: " + 
		...
		{
			System.out.println("Get X3D Selenium - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Selenium - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Selenium - View Hawkeye  1X");
