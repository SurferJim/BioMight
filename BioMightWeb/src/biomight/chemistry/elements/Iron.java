...
* @author SurferJim
*
* Representation of Iron
*
*************************************************************************/
public class Iron  extends BioMightBase {
	...
	public Iron()
	{
		...
		create(localVP, localLOD, Constants.IronRef, null, null);
	}

	public Iron(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Iron(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		System.out.println("Creating Iron for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Iron Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Iron,
			// Go get the details for the current Iron is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Iron				
				System.out.println("Getting the Iron MAG2X : " + parentID);
				...
				//System.out.println("In Iron - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Iron directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye IronInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Iron Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Iron");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Iron for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Iron NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Iron at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Iron				
					System.out.println("Creating Iron at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateIron Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Iron Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Iron");
		method.setMethodName("Penetrate Iron");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Iron : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Iron:01")) {
				...
				//int success = bioMightBean.generateIron("Iron:00001", "Iron", 
				//		"Iron", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Iron:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateIron("Iron:00160", "Iron", 
				//	"Iron", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Iron NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Iron");
			throw new ServerException("Remote Exception Iron():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Iron.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Iron.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Iron!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Iron - View Internal");
			// We do nada as the Iron Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Iron - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Iron: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Iron at Position: " + 
		...
		{
			System.out.println("Get X3D Iron - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Iron - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Iron - View Hawkeye  1X");
