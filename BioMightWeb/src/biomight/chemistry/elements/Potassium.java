...
* @author SurferJim
*
* Representation of Potassium
*
*************************************************************************/
public class Potassium  extends BioMightBase {
	...
	public Potassium()
	{
		...
		create(localVP, localLOD, Constants.PotassiumRef, null, null);
	}

	public Potassium(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Potassium(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Potassium.jpg");
		...
		System.out.println("Creating Potassium for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Potassium Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Potassium,
			// Go get the details for the current Potassium is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Potassium				
				System.out.println("Getting the Potassium MAG2X : " + parentID);
				...
				//System.out.println("In Potassium - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Potassium directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye PotassiumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Potassium Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Potassium");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Potassium for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Potassium NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Potassium at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Potassium				
					System.out.println("Creating Potassium at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreatePotassium Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Potassium Methods: " + bioMightMethods.size());
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Potassium : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Potassium:01")) {
				...
				//int success = bioMightBean.generatePotassium("Potassium:00001", "Potassium", 
				//		"Potassium", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Potassium:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generatePotassium("Potassium:00160", "Potassium", 
				//	"Potassium", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Potassium NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Potassium");
			throw new ServerException("Remote Exception Potassium():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Potassium.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the Potassium
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='Potassium.jpg'/>\n" +
		...
		{	
			...
			System.out.println("Get X3D Potassium - View Internal");
			// We do nada as the Potassium Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Potassium - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Potassium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Potassium at Position: " + 
		...
		{
			System.out.println("Get X3D Potassium - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Potassium - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Potassium - View Hawkeye  1X");
