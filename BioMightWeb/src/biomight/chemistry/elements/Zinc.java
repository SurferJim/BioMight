...
*
*************************************************************************/
public class Zinc  extends BioMightBase {
	...
	public Zinc()
	{
		...
		create(localVP, localLOD, Constants.ZincRef, null, null);
	}

	public Zinc(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Zinc(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		System.out.println("Creating Zinc for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Zinc Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Zinc,
			// Go get the details for the current Zinc is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Zinc				
				System.out.println("Getting the Zinc MAG2X : " + parentID);
				...
				//System.out.println("In Zinc - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Zinc directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye ZincInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Zinc Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Zinc");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Zinc for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Zinc NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Zinc at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Zinc				
					System.out.println("Creating Zinc at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateZinc Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Zinc Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Zinc");
		method.setMethodName("Penetrate Zinc");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Zinc : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Zinc:01")) {
				...
				//int success = bioMightBean.generateZinc("Zinc:00001", "Zinc", 
				//		"Zinc", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Zinc:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateZinc("Zinc:00160", "Zinc", 
				//	"Zinc", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Zinc NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Zinc");
			throw new ServerException("Remote Exception Zinc():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Zinc.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Zinc.jpg'/>\n" +
		...
		{	
			...
			System.out.println("Get X3D Zinc - View Internal");
			// We do nada as the Zinc Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Zinc - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Zinc: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Zinc at Position: " + 
		...
		{
			System.out.println("Get X3D Zinc - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Zinc - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Zinc - View Hawkeye  1X");
