...
*
*************************************************************************/
public class Iodine  extends BioMightBase {
	...
	public Iodine()
	{
		...
		create(localVP, localLOD, Constants.IodineRef, null, null);
	}

	public Iodine(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Iodine(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Iodine.jpg");
		...
		System.out.println("Creating Iodine for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Iodine Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Iodine,
			// Go get the details for the current Iodine is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Iodine				
				System.out.println("Getting the Iodine MAG2X : " + parentID);
				...
				//System.out.println("In Iodine - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Iodine directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye IodineInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Iodine Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Iodine");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Iodine for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Iodine NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Iodine at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Iodine				
					System.out.println("Creating Iodine at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateIodine Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Iodine Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Iodine");
		method.setMethodName("Penetrate Iodine");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Iodine : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Iodine:01")) {
				...
				//int success = bioMightBean.generateIodine("Iodine:00001", "Iodine", 
				//		"Iodine", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Iodine:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateIodine("Iodine:00160", "Iodine", 
				//	"Iodine", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Iodine NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Iodine");
			throw new ServerException("Remote Exception Iodine():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Iodine.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Iodine.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Iodine!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Iodine - View Internal");
			// We do nada as the Iodine Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Iodine - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Iodine: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Iodine at Position: " + 
		...
		{
			System.out.println("Get X3D Iodine - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Iodine - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Iodine - View Hawkeye  1X");
