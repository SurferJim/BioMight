 ...
 * @author SurferJim
 *
 * Representation of a Collection of Nickels
 * 
 ********************************************************************************/

public class Nickels extends BioMightBase {
private ArrayList nickels;

	
	/********************************************************************************************************************
	 *  Nickels
	 * 
	 * This method will instantiate the Nickels that are defined for the current model.  
	 * 
	 ********************************************************************************************************************/
	...
	public Nickels(String parentID)
	{				
	...
	public Nickels(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	...
	{
		this.setImage("images/Nickels.jpg");
		
		nickels = new ArrayList();
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Nickels Methods: " + bioMightMethods.size());
		...
		try {
			...
			System.out.println("Getting Nickels Transform: " + componentID + "   " +  parentID);
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			bioMightTransforms = bioMightBean.getComponents(Constants.NickelRef, parentID);   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - Nickels");
			throw new ServerException("Remote Exception getComponents():", e); 	
		}
		
		
		// Run through the collection of nickels and build them into the model
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Have Nickels NumTransforms: " + transforms.size());
		...
		String bioTemplate="Nickels.x3d";

		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the nickel we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Creating Nickel: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			
			// Create an instance of a Nickel for each tranform specified
			localVP = Constants.VIEW_INTERNAL;
			localLOD = Constants.MAG1X;
			
			Nickel nickel = new Nickel(localVP, localLOD, bioMightTransform.getId(), bioMightProperties, bioMightMethods);		
			System.out.println("Nickel Created!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			nickels.add(nickel);
			System.out.println("Added Nickel to Collection!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());

			// initialize the Properties
			// in this case, rather than pass canonical name, we pass the name found in the collectio
			initProperty(bioMightTransform.getName(), Constants.Nickel, Constants.NickelRef, bioMightTransform.getId());				
			//initProperty(bioMightTransform.getName(), Constants.Nickel, Constants.NickelRef, parentID);
		...
		// Set up methods that will be available to the nickels
	...
	public void initMethods(ArrayList<BioMightMethodView> bioMightMethodsIn) {
		...
		if (bioMightMethodsIn.size() == 0) {
			System.out.println("New Set of NickelsMethods: " + bioMightMethodsIn.size());
			BioMightMethodView method = new BioMightMethodView();
			method.setCanonicalName(Constants.Nickels);
		...
		{
			System.out.println("Using Existing NickelsMethods: " + bioMightMethodsIn.size());
	...
	public String getX3D(boolean snipet) {
		...
		{
			System.out.println("Assembling X3D for Nickels HawkEyeView - size: " + nickels.size());
			
			// Run through the collection of Nickels and assemble the X3D for each
			for (int i=0; i<nickels.size(); i++)
			{
				// Get the information for the nickel
				//Nickel nickel = (Nickel) nickels.get(i);
				//System.out.println("Getting X3D for Nickel: " + nickel.getComponentID());
				//body += nickel.getX3D(true);
			}		
			...
			for (int i=0; i<transforms.size(); i++)
			{
				...
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Getting X3D -InternalView - for Nickel: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());				

				//System.out.println("Getting X3D for NickelX: " + bioMightTransform.getTranslation().getXPos());
				//System.out.println("Getting X3D for NickelY: " + bioMightTransform.getTranslation().getYPos());
				//System.out.println("Getting X3D for NickelZ: " + bioMightTransform.getTranslation().getZPos());
		...
		{	
			System.out.println("Assembling X3D for Nickels InternalView - size: " + nickels.size());
		...
		//System.out.println("Nickels X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("Nickels-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for Nickels: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.Nickels)) {				
			...
			if (fired)
			{	
				System.out.println("Nickels - Methods have fired.   Calling Nickels Save method!");
			}
		}
	}
	 ...
	 * SET COLONY SIZE 
	 *
	 * This method will set the Colony size for the Nickels.  
	 *****************************************************************************************/
	public void setColonySize(int size) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("Nickels-SetColony Size: " + size);
		
		// Generate the Nickel		
		BioMightCellularBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Nickel Colony: " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			//bioMightBean = (BioMightCellularBeanLocal) ctx.lookup("biomight.ejb.BioMightCellularBeanLocal");
			bioMightBean = (BioMightCellularBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightCellularBean!biomight.ejb.BioMightCellularBeanLocal");
	
			double currentPoints[][] = null;
		//	int success = bioMightBean.generateNickels(size, "Nickel:00001", "Nickel", 
		//		"Nickel", this.componentID, this.parentID, currentPoints);
			
			System.out.println("Created Nickel Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Creating Colony - Nickel");
			throw new ServerException("Remote Exception NickelEpithelium():", e); 	
		}
	}
