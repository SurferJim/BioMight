...
* @author SurferJim
*
* Representation of Magnesium
*
*************************************************************************/
public class Magnesium  extends BioMightBase {
	...
	public Magnesium()
	{
		...
		create(localVP, localLOD, Constants.MagnesiumRef, null, null);
	}

	public Magnesium(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Magnesium(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Magnesium.jpg");
		...
		System.out.println("Creating Magnesium for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Magnesium Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Magnesium,
			// Go get the details for the current Magnesium is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Magnesium				
				System.out.println("Getting the Magnesium MAG2X : " + parentID);
				...
				//System.out.println("In Magnesium - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Magnesium directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye MagnesiumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Magnesium Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Magnesium");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Magnesium for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Magnesium NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Magnesium at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Magnesium				
					System.out.println("Creating Magnesium at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateMagnesium Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Magnesium Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Magnesium");
		method.setMethodName("Penetrate Magnesium");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Magnesium : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Magnesium:01")) {
				...
				//int success = bioMightBean.generateMagnesium("Magnesium:00001", "Magnesium", 
				//		"Magnesium", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Magnesium:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateMagnesium("Magnesium:00160", "Magnesium", 
				//	"Magnesium", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Magnesium NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Magnesium");
			throw new ServerException("Remote Exception Magnesium():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Magnesium.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the Magnesium
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='Magnesium.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Magnesium!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Magnesium - View Internal");
			// We do nada as the Magnesium Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Magnesium - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Magnesium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Magnesium at Position: " + 
		...
		{
			System.out.println("Get X3D Magnesium - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Magnesium - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Magnesium - View Hawkeye  1X");
