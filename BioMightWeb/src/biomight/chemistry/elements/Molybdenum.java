...
*
*************************************************************************/
public class Molybdenum  extends BioMightBase {
	...
	public Molybdenum()
	{
		...
		create(localVP, localLOD, Constants.MolybdenumRef, null, null);
	}

	public Molybdenum(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Molybdenum(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Molybdenum.jpg");
		...
		System.out.println("Creating Molybdenum for: " + parentID);
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Molybdenum Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Molybdenum,
			// Go get the details for the current Molybdenum is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Molybdenum				
				System.out.println("Getting the Molybdenum MAG2X : " + parentID);
				...
				//System.out.println("In Molybdenum - Capsid is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Molybdenum directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye MolybdenumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Molybdenum Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Molybdenum");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Molybdenum for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Molybdenum NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Molybdenum at HawkEye MAG1X - Just Init Properties: " + parentID);
				...
				{
					// Go get the finer details of the Molybdenum				
					System.out.println("Creating Molybdenum at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateMolybdenum Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Molybdenum Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Molybdenum");
		method.setMethodName("Penetrate Molybdenum");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Molybdenum : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Molybdenum:01")) {
				...
				//int success = bioMightBean.generateMolybdenum("Molybdenum:00001", "Molybdenum", 
				//		"Molybdenum", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Molybdenum:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateMolybdenum("Molybdenum:00160", "Molybdenum", 
				//	"Molybdenum", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Molybdenum NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Molybdenum");
			throw new ServerException("Remote Exception Molybdenum():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Molybdenum.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the Molybdenum
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='Molybdenum.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Molybdenum!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Molybdenum - View Internal");
			// We do nada as the Molybdenum Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Molybdenum - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Molybdenum: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Molybdenum at Position: " + 
		...
		{
			System.out.println("Get X3D Molybdenum - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Molybdenum - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += electrons.getX3D(true);	
			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Molybdenum - View Hawkeye  1X");
