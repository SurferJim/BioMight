 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class DescemetsMembrane extends BioMightBase {
	...
	public DescemetsMembrane()
	{
		create(Constants.DescemetsMembraneRef, null);
	}
	
	
	public DescemetsMembrane(String parentID)
	{
		create(parentID, null);
	}
	
	public DescemetsMembrane(String parentID, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		setImage("images/DescemetsMembrane.jpg");
		setImageHeight(300);
		setImageWidth(300);
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Getting DescemetsMembraneInfo for ParentID: " + parentID);
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			bioMightTransforms = bioMightBean.getComponents(Constants.DescemetsMembraneRef, parentID);
			System.out.println("Have DescemetsMembrane Info from EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - DescemetsMembrane");
		...
		System.out.println("Have DescemetsMembrane NumTransforms: " + transforms.size());
		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the eye we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Creating DescemetsMembrane (name,id): " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			
			int view = Constants.VIEW_FLOATING;
			if (view == Constants.VIEW_FLOATING)
			{
				//lips = new Lips(bioMightTransform.getId(), bioMightMethods);
				//System.out.println("In DescemetsMembrane - Created Lips");
		...
		initProperties();
		initMethods();
		System.out.println("Created DescemetsMembrane");
	}
	...
	public void redraw(String parentID)
	{
		...
		if (view == Constants.VIEW_FLOATING)
		{
			System.out.println("DescemetsMembrane - Redraw");
		}
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='DescemetsMembrane.jpg'/>\n" +
		...
		"<Scene>\n" +
		"<WorldInfo\n" +
		"title='DescemetsMembrane'\n" +
		"info='\"BioMight Generated X3D\"'/>\n";		

		System.out.println("Assembling DescemetsMembrane X3D");
		String body = "";
		
		//System.out.println("DescemetsMembrane X3D: " + body);		
	...
	public void Ingest()
	{
		...
		// placing food in one's descemetsMembrane
	}
