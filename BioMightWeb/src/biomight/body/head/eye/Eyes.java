 ...
 * @author SurferJim
 *
 * This class will instantiate the eyes that are defined for the body we
 * are working with.
 * 
 */

public class Eyes extends BioMightBase {
	...
	public Eyes()
	{
		...
		create(localVP, localLOD, Constants.EyesRef, null, null);
	}
	 ...
	 *
	 ***********************************************************************/
	public Eyes(String parentID)
	{
		//System.out.println("Calling parameterized Eyes Constructor!");
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	 ...
	 *
	 ***********************************************************************/
	public Eyes(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		System.out.println("Calling Eyes with LOD!");
	...
	public void create(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Getting EyesInfo for ParentID: " + parentID);
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			bioMightTransforms = bioMightBean.getComponents(Constants.EyeRef, parentID);
			System.out.println("Have Eyes Info from EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - Back");
			throw new ServerException("Remote Exception getComponents():", e); 	
		}
		
		// Get the collection of Eyes from the database.  
		//this.componentID = componentID;
		componentID = Constants.EyesRef+":0"; 
		
		// Run through the collection of Eyes and build them into the model
		// In the Default case, we get two instances of the eys, one
		// positioned on the right and one positioned on the left
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Have Eyes NumTransforms: " + transforms.size());
		for (int i=0; i<transforms.size(); i++)
		{
			...
			Eye eye = new Eye(localVP, localLOD, bioMightTransform.getId(), bioMightProperties, bioMightMethods);		
			System.out.println("Eye Created!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			eyes.add(eye);
	...
	public void initMethods() {
		...
		method.setDisplayName("Iris Color");
		...
		method.setDisplayName("Iris Texture");
		method.setHtmlType(Constants.BIO_DROPDOWN);
		method.setDataType(Constants.BIO_TEXT);
		...
		method.setCanonicalName(Constants.BioText);
		method.setMethodName("setTexture");
		method.setDisplayName("Pupil Texture");
		method.setHtmlType(Constants.BIO_DROPDOWN);
		method.setDataType(Constants.BIO_TEXT);
	 	method.setValueMap(textureDDMap);
		methods.add(method);
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Eyes.jpg'/>\n" +
		...
		String body = ""; 
		System.out.println("Assembling X3D for Eyes");
		...
		for (int i=0; i<eyes.size(); i++)
		{
			...
			body += eye.getX3D(true);
		}		
		

		//System.out.println("Eyes X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("Eyes-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for Eyes: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.Eyes)) {				
			...
			if (fired)
			{	
				System.out.println("EYES - Methods have fired.   Calling Eyes Save method!");
				//save();
			}
		}
	}
