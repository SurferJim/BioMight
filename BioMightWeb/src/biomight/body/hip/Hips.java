 ...
 * @author SurferJim
 *
 * This class will instantiate the Hips that are defined for the body we
 * are working with.
 * 
 */

public class Hips extends BioMightBase {
	private ArrayList hips;
	...
	public Hips()
	{
		...
		create(localVP, localLOD, Constants.HipsRef, null, null);
	}
	 ...
	 *
	 ***********************************************************************/
	public Hips(String parentID)
	{
		System.out.print("Calling parameterized Hips Constructor!");
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	 ...
	 *
	 ***********************************************************************/
	public Hips(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		System.out.print("Calling Hips with MethodParams!");
	...
	public void create(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		System.out.println("HIPS - Getting BoundBoxes & Connectors!");
		...
		else
		{
			...
			componentBoundBoxes = bioMightConstruct.getBoundingBoxes(Constants.HipsRef);			
		}
		
		System.out.println("HIPS - Setting up internal Bounding Boxes!");
		...
		BioMightGenerate generatedHips =  new BioMightGenerate();
		
		// Get the information from the database via the Enterprise Bean		
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Getting HipsInfo for ParentID: " + parentID);
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			bioMightTransforms = bioMightBean.getComponents(Constants.HipRef, parentID);
			System.out.println("Have Hips Info from EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - Hips");
			throw new ServerException("Remote Exception getComponents():", e); 	
		}
		
		// This is a collection, so set the parent of the children
		// to that of the aggregation object
		componentID = Constants.HipsRef+":0"; 
		
		// Run through the collection of Hips and build them into the model
		// In the Default case, we get two instances of the hip, one
		// positioned on the right and one positioned on the left
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Hips NumTransforms: " + transforms.size());
		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the hip we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Creating Hip: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			
			// HIPS
			// Create and Load Constructor object
			System.out.println("In Hips - Setting up Constructor");
			bioConstruct = new BioMightConstruct(); 	
		
			System.out.println("In Hips - Load the appropriate bound box: " + bioMightTransform.getId());			
			...
			if (tempBioMightBoundBoxes != null)
				System.out.println("In Hips - Constructor Loaded with: " + bioMightTransform.getId());
			else
				System.out.println("In Hips - Constructor NOT Loaded with: " + bioMightTransform.getId());	
			bioConstruct.setBoundingBoxes(bioMightTransform.getId(), tempBioMightBoundBoxes);
			System.out.println("In Hips - Constructor Loaded with BoundingBoxes: " + bioMightTransform.getId());
			
			// Create an instance of the Hips for each tranform specified for the organism
			Hip hip = new Hip(bioMightTransform.getId(), bioConstruct, bioMightMethods, bioMightProperties);		
			System.out.println("Hip Created!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			hips.add(hip);
		
			// Get BioGenerate object from the creation
			BioMightGenerate generatedHip = hip.getBioMightGenerate();
			// Add the  details to collection object, store it based on its ID
			generatedHips.setMapComponent(bioMightTransform.getId(), generatedHip);
			System.out.println("Add HipBioGen to HipsBioGen: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			
			BioMightBoundBox tempboundingBox =  hip.getBioMightBoundBox();
			...
			initProperty(bioMightTransform.getName(), Constants.Hip, Constants.HipRef, bioMightTransform.getId(), bioPos, dimensions, Constants.SINGLE_COMPONENT, false);		
			System.out.println("InitProperty: " + bioMightTransform.getName() + "  " + bioMightTransform.getId() + "  " + bioPos + "   " + dimensions);
			
			System.out.println("Add Hips to Collection!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
		}

		// Set up methods that will be available to the Hips
		initMethods();
	}
	...
	public String getX3D(boolean snipet) {
		...
		String body = ""; 
		System.out.println("Assembling X3D for Hips");
		
		// Run through the collection of Hips and assemble the X3D for each
		for (int i=0; i<hips.size(); i++)
		{
			// Get the information for the eye
			Hip hip = (Hip) hips.get(i);
			System.out.println("Getting X3D for Hip");
			body += hip.getX3D(true);
		}		
		

		//System.out.println("Hips X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("Hips-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for Hips: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.Hips)) {				
			...
			if (fired)
			{	
				System.out.println("Hips - Methods have fired.   Calling Hips Save method!");
				//save();
			}
		}
	}
	 ...
	 * 
	 * Setup the Default Bounding Boxes for the Hips.  The routine
	...
	private BioMightBoundBoxes setupDefaultBoundBoxes() 
	{
		...
		//********************************************************************* er
		// RIGHT HIP BOUNDBOX
		// Set up the Bounding Box for the Should
	 ...
	 * SETUP BOUND BOXES
	 * 
	 * Setup the Internal Bounding Boxes for the Hips. This divides
	 * the Hips into its vascular,muscular,tissue components   
	 *	
	...
	private HashMap setupBoundBoxes(BioMightBoundBoxes bioMightBoundBoxesIn) 
	{
		...
		if (bioMightBoundBoxesIn != null)
		{
			...
			BioMightBoundBox bioMightBoundBoxTemp = bioMightBoundBoxesIn.getBoundingBox(Constants.LeftHipRef);
			System.out.println("LeftHip - SetupBoundBoxes - Incoming BoundBox: " + 
					bioMightBoundBoxTemp.getXPos() + " " +
					bioMightBoundBoxTemp.getYPos() + " " +
					bioMightBoundBoxTemp.getZPos());

			// In the default instance,there will be two bound boxes
			bioMightBoundBoxTemp = bioMightBoundBoxesIn.getBoundingBox(Constants.RightHipRef);
			System.out.println("RightHip - SetupBoundBoxes - Incoming BoundBox: " + 
					bioMightBoundBoxTemp.getXPos() + " " +
					bioMightBoundBoxTemp.getYPos() + " " +
					bioMightBoundBoxTemp.getZPos());		
		}
		...
		{
			System.out.println("HipsSetupBoundBoxes - Incoming BoundBoes are Null");
		}
		...
		bioBoundBoxes.setBoundingBox(Constants.HipEpitheliumRef, bioBoundBox);
		
		// Put the BioMight BoundBoxes for the Left Hip into the BoundingBoxMap 
		boundingBoxesMap.put(Constants.LeftHipRef, bioBoundBoxes);
		System.out.println("Adding - Left Hip bioBoundBoxes into BoxesMap"); 

	
		bioBoundBoxes = new BioMightBoundBoxes();
			
		//********************************************************************* 
		// RIGHT HIP EPITHELIUM BOUNDBOX
		// Set up the Bounding Box for the Hips
		// The connector for this will come from the incoming Bound Box
		// Both are defined like and bolt and they lock into position at the
		// interestion of both connectors
		...
		// Put the BioMight BoundBoxes for the Left Hip into the BoundingBoxMap 
		boundingBoxesMap.put(Constants.RightHipRef, bioBoundBoxes);
		System.out.println("Adding - Right Hip bioBoundBoxes into BoxesMap"); 
