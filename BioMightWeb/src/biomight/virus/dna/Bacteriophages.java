...
public class Bacteriophages extends BioMightBase {
private ArrayList bacteriophages;
	...
	public Bacteriophages(String parentID)
	{				
	...
	public Bacteriophages(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	...
	public String getX3D(boolean snipet) {
		...
		if (viewPerspective == Constants.VIEW_HAWKEYE)
		{
			...
			for (int i=0; i<transforms.size(); i++)
			{
				...
				double neckHeight = 1.25;
				double neckDisplacement = 0.50;
				double neckRadius = 0.1;
			
				double fiberRadius = 0.05;
				double fiberLength = MathUtils.round(neckHeight/1.25, 8);
				
				double[] startPos = {0, 0, 0};
						//bioMightTransform.getTranslation().getXPos(), 
						//bioMightTransform.getTranslation().getYPos()-capHeight, 
						//bioMightTransform.getTranslation().getZPos()};
				double capTopPos[] = {startPos[0], (startPos[1]+capHeight), startPos[2]};
				...
				int numPoints = 6;
				double[][] cyPoints = BioGraphics.createCylinderInPlane(Constants.YPLANE, startPos, radius, numPoints);
				double[][] cyCapPoints = BioGraphics.createCylinderInPlane(Constants.YPLANE, startPos, outerRadius, numPoints);
				...
				String centerPointStr = capTopPos[0] + ", " + capTopPos[1] + ", "  + capTopPos[2]; 
				...
				startPos[1] = (startPos[1] - bodyHeight);
				double capBasePos[] = {startPos[0], (startPos[1] - capHeight), startPos[2]};
				double[][] cyBasePoints = BioGraphics.createCylinderInPlane(Constants.YPLANE, startPos, radius, numPoints);
				//double[][] cyBaseCapPoints = BioGraphics.createCylinderInPlane(Constants.XPLANE, startPos, outerRadius, numPoints);
						
				// Create 5 triangles and form them into an index faced set.
				// Use the Centerpoint of 0,0,0 and drop the startPos 
				// of the create Cylinder to determine the depth of the cap
				centerPointStr = capBasePos[0] + ", " + capBasePos[1] + ", " + capBasePos[2]; 
				...
				body+= " <Material DEF='Rust'\n" +
			 	...
			 	//***************************************************************
			 	// CREATE THE 'THIGH' LEGS
			 	//
				// Create coordinates for the inner leg segmments.  Tilt the legs
				// back outward
			 	//
			 	//****************************************************************
				double innerLegsRadius = 0.35;
				double innerLegPos[] = {0.0, -1.75, 0.0};
				//numPoints=6;
				double[][] innerLegPoints = BioGraphics.createCylinderInPlane(Constants.YPLANE, innerLegPos, innerLegsRadius, numPoints);
				...
				// back inward
			 	//
			 	//****************************************************************
				double outerLegsRadius = 0.95;
				double outerLegPos[] = {0.0, -1.625, 0.0};
				
				double sphereRadius = 0.625;
				double spherePos[] = {0.0, -1.275, 0.0};
				
				//numPoints=6;
				double[][] outerLegPoints = BioGraphics.createCylinderInPlane(Constants.YPLANE, outerLegPos, outerLegsRadius, numPoints);
				double[][] spherePoints = BioGraphics.createCylinderInPlane(Constants.YPLANE, spherePos, sphereRadius, numPoints);
					
				angleLatitude = 60;
				radiansLatitude =  Math.toRadians(angleLatitude);
				totalAngleLatitude = 0;
				...
				for (int k=0; k<outerLegPoints.length; k++)
				{
					System.out.println("InnerLeg Coordinates for : + " + k + "\n" 
				        + "XPOS: " + outerLegPoints[k][0]  + "\n" 
		 				+ "YPOS: " + outerLegPoints[k][1]  + "\n" 
		 				+ "ZPOS: " + outerLegPoints[k][2] + "\n");			
					...
					System.out.println("outerLegPoints Axis for : + " + k);
					System.out.println("outerLegPoints X-AXIS: " + xOrient);
					System.out.println("outerLegPoints Y-AXIS: " + yOrient);
					System.out.println("outerLegPoints Z-AXIS: " + zOrient);
					
					
					double degrees = -radiansLongitude;
					System.out.println("outerLegPoints RotateAngle: " + angleLatitude + "  degrees: " +  degrees);
