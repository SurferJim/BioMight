 ...
 * @author SurferJim
 *
 * Representation of a GlycoProteinSpike.
 * 
 ************************************************************************************/
...
public class GlycoProteinSpike extends BioMightBase {
	...
	/************************************************************************
	 * GlycoProteinSpike Constructor 
	 *
	 ***********************************************************************/
	public GlycoProteinSpike()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.GlycoProteinSpikeRef, null, null);
	}
	 ...
	 *
	 ***********************************************************************/
	public GlycoProteinSpike (String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	 ...
	 *
	 ***********************************************************************/
	public GlycoProteinSpike(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	/************************************************************************
	 * Create GlycoProteinSpike
	 *
	 ***********************************************************************/

	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
		this.setImage("images/GlycoProteinSpike.jpg");
		...
		String bioTemplate="GlycoProteinSpike.x3d";
		...
		{			
			...
			{
				//System.out.println("Creating GlycoProteinSpike at MAG1X: " + parentID);
				// initialize the Properties
				initProperties();
			}
			else if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the GlycoProteinSpike				
				//System.out.println("Creating GlycoProteinSpike at MAG2X: " + parentID);
				localVP = Constants.VIEW_HAWKEYE; 
				localLOD = Constants.MAG1X;

				//System.out.println("In GlycoProteinSpike- Creating Capsomeres");
		...
		{			
			...
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting GlycoProteinSpikeInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponents(Constants.GlycoProteinSpikeRef, parentID);
				System.out.println("Have GlycoProteinSpikeInfo from EJB");   	
			}catch (Exception e) { 
			...
			System.out.println("Have GlycoProteinSpikeTransforms : " + transforms.size());
			...
			{	
				...
				{
					// Go get the finer details of the GlycoProteinSpike				
					System.out.println("Creating GlycoProteinSpike: " + parentID + " lod: " + localLOD);
					localVP = Constants.VIEW_HAWKEYE; 
					localLOD = Constants.MAG1X;
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a GlycoProteinSpikedirectly
			// We have the ID of the component through the Drill-down
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye GlycoProteinSpikeInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			...
			// In the default case, we get one instance of the GlycoProteinSpikefor each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("GlycoProteinSpikeNumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the GlycoProteinSpike
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating GlycoProteinSpike: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();
				...
				{
					// Go get the finer details of the GlycoProteinSpike				
					System.out.println("Creating GlycoProteinSpike: " + parentID + " at MAG2X");
					localVP = Constants.VIEW_HAWKEYE; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("Create GlycoProteinSpikeSegmentComplete: " + parentID);
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the GlycoProteinSpike: " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			System.out.println("Calling Generate GlycoProteinSpike: " + componentID + "    " + parentID);
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the GlycoProteinSpike.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the GlycoProteinSpike
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='GlycoProteinSpike.jpg'/>\n" +
		...
		if (view == Constants.VIEW_INTERNAL)
		{
			...
			{
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("In GlycoProteinSpike- Getting X3D - MAG1X: " + transforms.size());
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
					//System.out.println("Getting GlycoProteinSpikeTailTail X3D: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			...
			{
				// Run through the collection of GlycoProteinSpikes and build them into the model
				// In the default case, we get one instance of the GlycoProteinSpikeTailfor each eye
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("In GlycoProteinSpike- Getting X3D - MAG2X: " + transforms.size());
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					body += "<Transform DEF='GlycoProteinSpike'\n" +
					...
					"<Shape DEF='" + "GlycoProteinSpike" + "'\n" +
					 ...
					 body+= " <Material DEF='Rust'\n" +
					    ...
					    " description='" + "GlycoProteinSpike" + "'\n" +
			           " containerField='children'/> \n" +
							    	 
					"</Transform>\n"; 
					 //System.out.println("GlycoProteinSpikeInterim X3D: " + body);	
				}
			}
		...
		body+= "<Viewpoint DEF='Viewpoint_GlycoProteinSpike'\n" +
		...
		//System.out.println("GlycoProteinSpikeX3D: " + body);	
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("GlycoProteinSpike-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for GlycoProteinSpike: " + bioMightMethod.getMethodName());	
			...
			// We only execute those methods that are targeted for the GlycoProteinSpike
			// If a parameter is specified then we fire the method, otherwise
			// we just jump over it
			if (canonicalName.equals(Constants.GlycoProteinSpike)) {				
			...
			if (fired)
			{	
				System.out.println("Methods have fired.   Calling GlycoProteinSpikeSave method!");
				save();
			}
		}
	}
	 ...
	 * SET Radius
	 * 
	 * Set the size of the GlycoProteinSpike
	 * 
	 *****************************************************************************/
	public void setRadius(double size) {
		...
		System.out.println("Setting GlycoProteinSpikeRadius: " + size);
		ArrayList transforms = bioMightTransforms.getTransforms();
		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the pupil we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Updating GlycoProteinSpikeRadius: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			bioMightTransform.setRadius(size);
			
			// Store the updated information GlycoProteinSpikeinto the transformation
			transforms.set(i,bioMightTransform);
			System.out.println("Updated GlycoProteinSpikeRadius");
		}		
	...
	/*****************************************************************************
	 * SET COLOR - Sets the color of the GlycoProteinSpike
	 * 
	 * Set the color of the GlycoProteinSpike
	 * 
	 *****************************************************************************/
	public void setColor(String colorStr) {
		...
		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the pupil we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Setting GlycoProteinSpikeEyeColor: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			...
			System.out.println("Setting GlycoProteinSpikeColor " + bioMightTransform.getName() + "  to: " + colorStr);
	 ...
	 * SAVE 
	 * 
	 * This mmethod writes the Transform information GlycoProteinSpikeinto the database.
	...
	private void save() {		
		...
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Saving Tranforms for GlycoProteinSpike: " +  transforms.size());
		for (int i=0; i<transforms.size(); i++) {
			// Get the information for the pupil we are creating
			bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Updating GlycoProteinSpikeTranforms: " + bioMightTransform.getName() + "  " + bioMightTransform.getId()); 
			//System.out.println("GlycoProteinSpikeTranforms Radius is: " + bioMightTransform.getName() + "  " + bioMightTransform.getRadius());
		}
		
		// Get the information from the database via the Enterprise Bean				
		try {
			// Save the information via the update call	
			System.out.println("Saving GlycoProteinSpikeData");
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			int returnCode = bioMightBean.updateComponent(Constants.GlycoProteinSpikeRef, parentID, bioMightTransform);

			System.out.println("Saved GlycoProteinSpikeData!");  	
		}catch (Exception e) { 
