 ...
 * @author SurferJim
 *
 * Representation of a CapsidNeck.
 * 
 ************************************************************************************/
...
public class CapsidNeck extends BioMightBase {
	 ...
	 *
	 ***********************************************************************/
	public CapsidNeck (String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	 ...
	 *
	 ***********************************************************************/
	public CapsidNeck(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{		
		...
		String bioTemplate="CapsidNeck.x3d";
		...
		{			
			...
			try {
				...
				bioMightTransforms = bioMightBean.getComponents(Constants.CapsidNeckRef, parentID);
				System.out.println("Have CapsidNeckInfo from EJB");   	
			}catch (Exception e) { 
			...
			System.out.println("Have CapsidNeckTransforms : " + transforms.size());
			...
			{	
				...
				else if (localLOD == Constants.MAG2X)
				{
					...
					System.out.println("Creating CapsidNeck: " + parentID + " lod: " + localLOD);
					localVP = Constants.VIEW_HAWKEYE; 
					localLOD = Constants.MAG1X;

					//System.out.println("In CapsidNeck- Creating Capsomeres");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a CapsidNeckdirectly
			// We have the ID of the component through the Drill-down
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye CapsidNeckInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				...
				System.out.println("Exception Getting Components - CapsidNeck");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of Capsids and build them into the model
			// In the default case, we get one instance of the CapsidNeckfor each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("CapsidNeckNumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the Capsid
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating CapsidNeck: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();
				...
				{
					// Go get the finer details of the CapsidNeck				
					System.out.println("Creating CapsidNeck: " + parentID + " lod: " + localLOD);
					localVP = Constants.VIEW_HAWKEYE; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("Create CapsidNeckComplete: " + parentID);
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the CapsidNeck: " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			// Create 5 sections
			double[] startPos = {0.0, -0.20, 0.00};
			double[][] currentPoints = BioGraphics.octogonYPlane(startPos, circumference);
	      		
			System.out.println("Calling Generate CapsidNeck: " + componentID + "    " + parentID);
			int success = bioMightBean.generateCapsidNeck("", "CapsidNeck", 
				"CapsidNeck", componentID, parentID, currentPoints);			
			
			System.out.println("Created CapsidNeckInfo using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - CapsidNeck");
			throw new ServerException("Remote Exception getComponents():", e); 	
		}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the CapsidNeck.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the CapsidNeck
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='CapsidNeck.jpg'/>\n" +
		...
		if (viewPerspective == Constants.VIEW_INTERNAL)
		{
			if (lod == Constants.MAG2X)
			{
				...
				System.out.println("In CapsidNeck - Getting X3D - INTERNAL 2X - NADA DONE");
			}
			else if (lod == Constants.MAG1X)
			{
				// Run through the collection of CapsidNecks and build them into the model
				// In the default case, we get one instance of the CapsidNeck for each eye
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("In CapsidNeck - Getting X3D -INTERNAL 1X : " + transforms.size());
				
				for (int i=0; i<transforms.size(); i++)
				{
					// Get the information for the sclera we are creating
					BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
					//System.out.println("Getting CapsidNeck X3D: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					...
					"<Shape DEF='" + "CapsidNeck" + "'\n" +
					 ...
					 body+= " <Material DEF='Rust'\n" +
					    ...
					    " description='" + "CapsidNeck" + "'\n" +
			           " containerField='children'/> \n" +
							    	 
					"</Transform>\n"; 
					 //System.out.println("CapsidNeck Interim X3D: " + body);	
				}
			}
		...
		else if (viewPerspective == Constants.VIEW_HAWKEYE)
		{
			...
			{
				// Go get the X3D from the subcomponents
				System.out.println("In CapsidNeck - HawkEye - MAG2X Getting X3D");
			}
			else if (lod == Constants.MAG1X)
			{
				// Run through the collection of CapsidNecks and build them into the model
				// In the default case, we get one instance of the CapsidNeck for each eye
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("In CapsidNeck - HawkEye - MAG1X Getting X3D: " + transforms.size());
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					"<Shape DEF='" + "CapsidNeck" + "'\n" +
					 ...
					 body+= " <Material DEF='Rust'\n" +
						    ...
						    bioMightTransform.getMaterial().getDiffuseColor().getBlue() + "'/>\n" +
						 	"</Appearance>\n" +			    
						 	"<IndexedFaceSet DEF='CapsidNeckMatrixIFS' \n" +
						    	   "containerField='geometry' \n" +
						    	   "creaseAngle='" +  bioMightTransform.getCreaseAngle() + "'\n" +
						    	   "coordIndex='" + bioMightTransform.getCoordindexStr() + "'>" +
						    	   "<Coordinate DEF='CapsidNeckMatrix_Coord' \n" +
						    	    "containerField='coord' point='" +  bioMightTransform.getCoordpointStr() + "'/>" +
						    	  "</IndexedFaceSet>\n" +
						    	 "</Shape>\n" +
		...
		//System.out.println("CapsidNeckX3D: " + body);	
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("CapsidNeck-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for CapsidNeck: " + bioMightMethod.getMethodName());	
			...
			// We only execute those methods that are targeted for the CapsidNeck
			// If a parameter is specified then we fire the method, otherwise
			// we just jump over it
			if (canonicalName.equals(Constants.CapsidNeck)) {				
			...
			if (fired)
			{	
				System.out.println("Methods have fired.   Calling CapsidNeckSave method!");
				save();
			}
		}
	}
	 ...
	 * SET Radius
	 * 
	 * Set the size of the CapsidNeck
	 * 
	 *****************************************************************************/
	public void setRadius(double size) {
		...
		System.out.println("Setting CapsidNeckRadius: " + size);
		ArrayList transforms = bioMightTransforms.getTransforms();
		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the pupil we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Updating CapsidNeckRadius: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			bioMightTransform.setRadius(size);
			
			// Store the updated information CapsidNeckinto the transformation
			transforms.set(i,bioMightTransform);
			System.out.println("Updated CapsidNeckRadius");
		}		
	...
	/*****************************************************************************
	 * SET COLOR - Sets the color of the CapsidNeck
	 * 
	 * Set the color of the CapsidNeck
	 * 
	 *****************************************************************************/
	public void setColor(String colorStr) {
		...
		for (int i=0; i<transforms.size(); i++)
		{
			// Get the information for the pupil we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Setting CapsidNeckEyeColor: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			...
			System.out.println("Setting CapsidNeckColor " + bioMightTransform.getName() + "  to: " + colorStr);
	 ...
	 * SAVE 
	 * 
	 * This mmethod writes the Transform information CapsidNeckinto the database.
	...
	private void save() {		
		...
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Saving Tranforms for CapsidNeck: " +  transforms.size());
		for (int i=0; i<transforms.size(); i++) {
			// Get the information for the pupil we are creating
			bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Updating CapsidNeckTranforms: " + bioMightTransform.getName() + "  " + bioMightTransform.getId()); 
			//System.out.println("CapsidNeckTranforms Radius is: " + bioMightTransform.getName() + "  " + bioMightTransform.getRadius());
		}
		
		// Get the information from the database via the Enterprise Bean				
		try {
			// Save the information via the update call	
			System.out.println("Saving CapsidNeckData");
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			int returnCode = bioMightBean.updateComponent(Constants.CapsidNeckRef, parentID, bioMightTransform);
			System.out.println("Saved CapsidNeckData!");  	
		}catch (Exception e) { 
