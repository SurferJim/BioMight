...
public class RetroVirus extends BioMightBase {
	
	private String bacteriaName = "RetroVirus";
	...
	public RetroVirus()
	{
		...
		create(localVP, localLOD, Constants.RetroVirusRef, null, null);
	}

	public RetroVirus(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public RetroVirus(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In RetroVirus Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of RetroVirus,
			// Go get the details for the current RetroVirus is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the RetroVirus				
				System.out.println("Getting the RetroVirus MAG2X : " + parentID);
				...
				System.out.println("In RetroVirus - Capsid is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.RetroVirus, Constants.RetroVirusRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a RetroVirus directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye RetroVirusInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have RetroVirus Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the RetroVirus for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("RetroVirus NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the RetroVirus
				...
				{
					System.out.println("Creating RetroVirus at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the RetroVirus				
					System.out.println("Creating RetroVirus at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateRetroVirus Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate RetroVirus");
		method.setMethodName("Penetrate RetroVirus");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the RetroVirus : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateRetroVirus("RetroVirus:00001", "RetroVirus", 
			//		"RetroVirus", componentID, parentID, currentPoints);			
			
			System.out.println("Created RetroVirus Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - RetroVirus");
			throw new ServerException("Remote Exception RetroVirus():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the RetroVirus.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='RetroVirus.jpg'/>\n" +
		...
		System.out.println("Getting X3D for RetroVirus!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D RetroVirus - View Internal");
			// We do nada as the RetroVirus Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D RetroVirus - View Internal - 2X");
			    ...
			    body+= BioWebX3DUtils.generateRetroVirus(bioMightTransform, bioRange);
			    
			    body+= BioWebX3DUtils.generateRetroVirusCapsid(bioMightTransform,  bioMightPosition, bioMightOrientation); 	
			...
			{			
				...
				/*
				System.out.println("Creating RetroVirus: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating RetroVirus at Position: " + 
		...
		{
			System.out.println("Get X3D RetroVirus - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D RetroVirus - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				//body += cellMembrane.getX3D(true);
				
				//System.out.println("Get X3D RetroVirus - Getting Antibodies - 2X");
				//body += antibodies.getX3D(true);
				//System.out.println("Get X3D RetroVirus - Have Anitbodies - 2X");
				...
				//ranges.add(bioRange);
			  	...
			  	//body += BioWebX3DUtils.generateRetroVirus(bioMightTransform,  bioMightPosition, bioMightOrientation); 	

			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D RetroVirus - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					System.out.println("Creating RetroVirus at Position: " + 
						...
						for (int k=0; k<2; k++) {
							...
							bioMightTransform.setComponentName("Retro Virus");
							bioMightTransform.setTextureID(5);
							bioMightTransform.setTextureFile("SpeckledAmethyst.png");
							bioMightTransform.setRadius(0.75);
							body += BioWebX3DUtils.generateSphereSimple(bioMightTransform);	
		...
		body+= "<Viewpoint DEF='Viewpoint_RetroVirus'\n" +
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			if (fired)
			{	
				System.out.println("RetroViruss - Methods have fired.   Calling RetroViruss Save method!");
			}
		}
	}
	...
	{
		// RetroVirus Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// RetroVirus Bacteria hemolyzes Red Blood Cells
		if (obj instanceof RetroVirus)
		{
		}
