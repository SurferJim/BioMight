...
public class NoroVirus extends BioMightBase {
	
	private String bacteriaName = "NoroVirus";
	...
	public NoroVirus()
	{
		...
		create(localVP, localLOD, Constants.NoroVirusRef, null, null);
	}

	public NoroVirus(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public NoroVirus(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/NoroVirus.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In NoroVirus Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of NoroVirus,
			// Go get the details for the current NoroVirus is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the NoroVirus				
				System.out.println("Getting the NoroVirus MAG2X : " + parentID);
				...
				System.out.println("In NoroVirus - Capsid is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.NoroVirus, Constants.NoroVirusRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a NoroVirus directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye NoroVirusInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have NoroVirus Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - NoroVirus");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the NoroVirus for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("NoroVirus NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the NoroVirus
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				this.setBioMightTransform(bioMightTransform);
				System.out.println("Creating NoroVirus: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating NoroVirus at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the NoroVirus				
					System.out.println("Creating NoroVirus at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateNoroVirus Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING NoroVirus Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate NoroVirus");
		method.setMethodName("Penetrate NoroVirus");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the NoroVirus		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the NoroVirus : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateNoroVirus("NoroVirus:00001", "NoroVirus", 
			//		"NoroVirus", componentID, parentID, currentPoints);			
			
			System.out.println("Created NoroVirus Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - NoroVirus");
			throw new ServerException("Remote Exception NoroVirus():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the NoroVirus.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assemble the NoroVirus
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='NoroVirus.jpg'/>\n" +
		...
		System.out.println("Getting X3D for NoroVirus!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D NoroVirus - View Internal");
			// We do nada as the NoroVirus Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D NoroVirus - View Internal - 2X");
			    ...
			    body+= BioWebX3DUtils.generateNoroVirus(bioMightTransform, bioRange);
			...
			{			
				...
				/*
				System.out.println("Creating NoroVirus: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating NoroVirus at Position: " + 
		...
		{
			System.out.println("Get X3D NoroVirus - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D NoroVirus - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				//body += cellMembrane.getX3D(true);
				
				//System.out.println("Get X3D NoroVirus - Getting Antibodies - 2X");
				//body += antibodies.getX3D(true);
				//System.out.println("Get X3D NoroVirus - Have Anitbodies - 2X");
				...
				//ranges.add(bioRange);
			  	...
			  	//body += BioWebX3DUtils.generateNoroVirus(bioMightTransform,  bioMightPosition, bioMightOrientation); 	

			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D NoroVirus - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					System.out.println("Creating NoroVirus at Position: " + 
						...
						for (int k=0; k<2; k++) {
							...
							bioMightTransform.setComponentName("Noro Virus");
							bioMightTransform.setTextureID(5);
							bioMightTransform.setTextureFile("SpeckledAmethyst.png");
							bioMightTransform.setRadius(0.75);
							body += BioWebX3DUtils.generateSphereSimple(bioMightTransform);	
		...
		body+= "<Viewpoint DEF='Viewpoint_NoroVirus'\n" +
		...
		//System.out.println("NoroVirus X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for NoroVirus: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.NoroVirus)) {				
			...
			if (fired)
			{	
				System.out.println("NoroViruss - Methods have fired.   Calling NoroViruss Save method!");
			}
		}
	}
	...
	{
		// NoroVirus Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// NoroVirus Bacteria hemolyzes Red Blood Cells
		if (obj instanceof NoroVirus)
		{
		}
