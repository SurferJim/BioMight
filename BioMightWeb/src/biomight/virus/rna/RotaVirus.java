...
public class RotaVirus extends BioMightBase {
	
	private String bacteriaName = "RotaVirus";
	...
	public RotaVirus()
	{
		...
		create(localVP, localLOD, Constants.RotaVirusRef, null, null);
	}

	public RotaVirus(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public RotaVirus(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In RotaVirus Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of RotaVirus,
			// Go get the details for the current RotaVirus is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the RotaVirus				
				System.out.println("Getting the RotaVirus MAG2X : " + parentID);
				...
				System.out.println("In RotaVirus - Capsid is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.RotaVirus, Constants.RotaVirusRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a RotaVirus directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye RotaVirusInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have RotaVirus Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the RotaVirus for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("RotaVirus NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the RotaVirus
				...
				{
					System.out.println("Creating RotaVirus at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the RotaVirus				
					System.out.println("Creating RotaVirus at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateRotaVirus Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate RotaVirus");
		method.setMethodName("Penetrate RotaVirus");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the RotaVirus : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateRotaVirus("RotaVirus:00001", "RotaVirus", 
			//		"RotaVirus", componentID, parentID, currentPoints);			
			
			System.out.println("Created RotaVirus Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - RotaVirus");
			throw new ServerException("Remote Exception RotaVirus():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the RotaVirus.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='RotaVirus.jpg'/>\n" +
		...
		System.out.println("Getting X3D for RotaVirus!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D RotaVirus - View Internal");
			// We do nada as the RotaVirus Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D RotaVirus - View Internal - 2X");
			    ...
			    body+= BioWebX3DUtils.generateRotaVirus(bioMightTransform, bioRange);
			...
			{			
				...
				/*
				System.out.println("Creating RotaVirus: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating RotaVirus at Position: " + 
		...
		{
			System.out.println("Get X3D RotaVirus - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D RotaVirus - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				//body += cellMembrane.getX3D(true);
				
				//System.out.println("Get X3D RotaVirus - Getting Antibodies - 2X");
				//body += antibodies.getX3D(true);
				//System.out.println("Get X3D RotaVirus - Have Anitbodies - 2X");
				...
				//ranges.add(bioRange);
			  	...
			  	//body += BioWebX3DUtils.generateRotaVirus(bioMightTransform,  bioMightPosition, bioMightOrientation); 	

			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D RotaVirus - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					System.out.println("Creating RotaVirus at Position: " + 
						...
						for (int k=0; k<2; k++) {
							...
							bioMightTransform.setComponentName("Rota Virus");
							bioMightTransform.setTextureID(5);
							bioMightTransform.setTextureFile("SpeckledAmethyst.png");
							bioMightTransform.setRadius(0.75);
							body += BioWebX3DUtils.generateSphereSimple(bioMightTransform);	
		...
		body+= "<Viewpoint DEF='Viewpoint_RotaVirus'\n" +
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			if (fired)
			{	
				System.out.println("RotaViruss - Methods have fired.   Calling RotaViruss Save method!");
			}
		}
	}
	...
	{
		// RotaVirus Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// RotaVirus Bacteria hemolyzes Red Blood Cells
