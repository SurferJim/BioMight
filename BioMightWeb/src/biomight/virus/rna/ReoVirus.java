...
public class ReoVirus extends BioMightBase {
	
	private String bacteriaName = "ReoVirus";
	...
	public ReoVirus()
	{
		...
		create(localVP, localLOD, Constants.ReoVirusRef, null, null);
	}

	public ReoVirus(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public ReoVirus(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In ReoVirus Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of ReoVirus,
			// Go get the details for the current ReoVirus is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the ReoVirus				
				System.out.println("Getting the ReoVirus MAG2X : " + parentID);
				...
				System.out.println("In ReoVirus - Capsid is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.ReoVirus, Constants.ReoVirusRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a ReoVirus directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye ReoVirusInfo for ParentID: " + parentID);
			...
			System.out.println("ReoVirus NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the ReoVirus
				...
				{
					System.out.println("Creating ReoVirus at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the ReoVirus				
					System.out.println("Creating ReoVirus at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
		...
		System.out.println("CreateReoVirus Completed");
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the ReoVirus : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateReoVirus("ReoVirus:00001", "ReoVirus", 
			//		"ReoVirus", componentID, parentID, currentPoints);			
			
			System.out.println("Created ReoVirus Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - ReoVirus");
			throw new ServerException("Remote Exception ReoVirus():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the ReoVirus.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		System.out.println("Getting X3D for ReoVirus!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D ReoVirus - View Internal");
			// We do nada as the ReoVirus Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D ReoVirus - View Internal - 2X");
			    ...
			    body+= BioWebX3DUtils.generateReoVirus(bioMightTransform, bioRange);
		...
		{
			System.out.println("Get X3D ReoVirus - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D ReoVirus - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				//body += cellMembrane.getX3D(true);
				
				//System.out.println("Get X3D ReoVirus - Getting Antibodies - 2X");
				//body += antibodies.getX3D(true);
				//System.out.println("Get X3D ReoVirus - Have Anitbodies - 2X");
				...
				//ranges.add(bioRange);
			  	...
			  	//body += BioWebX3DUtils.generateReoVirus(bioMightTransform,  bioMightPosition, bioMightOrientation); 	

			}
			else
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D ReoVirus - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					System.out.println("Creating ReoVirus at Position: " + 
						...
						for (int k=0; k<2; k++) {
							...
							bioMightTransform.setComponentName("Reo Virus");
							bioMightTransform.setTextureID(5);
							bioMightTransform.setTextureFile("SpeckledAmethyst.png");
							bioMightTransform.setRadius(0.75);
							body += BioWebX3DUtils.generateSphereSimple(bioMightTransform);	
		...
		body+= "<Viewpoint DEF='Viewpoint_ReoVirus'\n" +
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			if (fired)
			{	
				System.out.println("ReoViruss - Methods have fired.   Calling ReoViruss Save method!");
			}
		}
	}
	...
	{
		// ReoVirus Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// ReoVirus Bacteria hemolyzes Red Blood Cells
		if (obj instanceof ReoVirus)
		{
		}
