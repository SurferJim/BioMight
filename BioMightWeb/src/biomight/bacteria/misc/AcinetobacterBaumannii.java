...
public class AcinetobacterBaumannii extends Bacteria {
	private String bacteriaName = "AcinetobacterBaumanniis";
	...
	public AcinetobacterBaumannii()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.AcinetobacterBaumanniisRef, null, null);
	}

	public AcinetobacterBaumannii(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public AcinetobacterBaumannii(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In AcinetobacterBaumanniis Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of AcinetobacterBaumanniis,
			// Go get the details for the current AcinetobacterBaumanniis is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the AcinetobacterBaumanniis				
				System.out.println("Getting the AcinetobacterBaumanniis MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("AcinetobacterBaumanniis Instance created from SubComponents : " + parentID);
				...
				System.out.println("In AcinetobacterBaumanniis - Fimbriae is complete");
				...
				System.out.println("In AcinetobacterBaumanniis - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.AcinetobacterBaumanniis, Constants.AcinetobacterBaumanniisRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a AcinetobacterBaumanniis directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye AcinetobacterBaumanniisInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have AcinetobacterBaumanniis Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the AcinetobacterBaumanniis for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("AcinetobacterBaumanniis NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating AcinetobacterBaumanniis at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the AcinetobacterBaumanniis				
					System.out.println("Creating AcinetobacterBaumanniis at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In AcinetobacterBaumanniis - CellMembrane is complete");
					...
					System.out.println("In AcinetobacterBaumanniis - Ribosomes are complete");
		...
		System.out.println("CreateAcinetobacterBaumanniis Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate AcinetobacterBaumanniis");
		method.setMethodName("Penetrate AcinetobacterBaumanniis");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the AcinetobacterBaumanniis : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("AcinetobacterBaumanniis:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateAcinetobacterBaumanniis("AcinetobacterBaumanniis:00001", "AcinetobacterBaumanniis", 
				//		"AcinetobacterBaumanniis", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("AcinetobacterBaumanniis:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateAcinetobacterBaumanniis("AcinetobacterBaumanniis:00160", "AcinetobacterBaumanniis", 
				//	"AcinetobacterBaumanniis", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate AcinetobacterBaumanniis NoParent");		
			}
			...
			System.out.println("Exception Getting Components - AcinetobacterBaumanniis");
			throw new ServerException("Remote Exception AcinetobacterBaumanniis():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the AcinetobacterBaumanniis.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the AcinetobacterBaumanniis
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='AcinetobacterBaumanniis.jpg'/>\n" +
		...
		System.out.println("Getting X3D for AcinetobacterBaumanniis!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D AcinetobacterBaumanniis - View Internal");
			// We do nada as the AcinetobacterBaumanniis Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D AcinetobacterBaumanniis - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating AcinetobacterBaumanniis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating AcinetobacterBaumanniis at Position: " + 
				...
				body+= " <Material DEF='Rust'\n" +
					...
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='AcinetobacterBaumanniis'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D AcinetobacterBaumanniis - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D AcinetobacterBaumanniis - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D AcinetobacterBaumanniis - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					for (int k=0; k<4; k++) {
						...
						body+= " <ImageTexture containerField='texture' " +
						    " url='../images/AcinetobacterBaumannii.jpg'/>";
						...
						body+= " <Material DEF='Rust'\n" +
						    ...
						    "diffuseColor='" + 
						 	    ...
						 	    bioMightTransform.getMaterial().getDiffuseColor().getBlue() + "'/>\n" +
						 	"</Appearance>\n" +
						 	"<Cylinder DEF='AcinetobacterBaumanniiGeoSphere'\n" +
						 	"containerField='geometry'\n" +
						 	"radius='" + bioMightTransform.getRadius() +" '\n" +
						...
						for (int l=0; l<2; l++) {
							...
							body+= " <Material DEF='Rust'\n" +
							    ...
							    "diffuseColor='" + 
							 	    ...
							 	    bioMightTransform.getMaterial().getDiffuseColor().getBlue() + "'/>\n" +
							 	"</Appearance>\n" +
							 	"<Sphere DEF='AcinetobacterBaumanniiGeoSphere'\n" +
							 	"containerField='geometry'\n" +
							 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
							 	"</Shape>\n" +
	...
	{
		// AcinetobacterBaumanniis Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// AcinetobacterBaumanniis Bacteria hemolyzes Red Blood Cells
