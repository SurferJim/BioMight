 ...
 * @author SurferJim
 *
 * Representation of StreptococcusPyogenes
 */

public class StreptococcusPyogenes extends Bacteria {
	private String bacteriaName = "StreptococcusPyogenes";
...
private GlutamicAcid glutamicAcid;
	...
	public StreptococcusPyogenes()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusPyogenesRef, null, null);
	}

	public StreptococcusPyogenes(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusPyogenes(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusPyogenes.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusPyogenes Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusPyogenes,
			// Go get the details for the current StreptococcusPyogenes is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusPyogenes				
				System.out.println("Getting the StreptococcusPyogenes MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusPyogenes Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusPyogenes - Fimbriae is complete");
				...
				System.out.println("In StreptococcusPyogenes - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StreptococcusPyogenes, Constants.StreptococcusPyogenesRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusPyogenes directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusPyogenesInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusPyogenes Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusPyogenes");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusPyogenes for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusPyogenes NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusPyogenes
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusPyogenes: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusPyogenes at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusPyogenes				
					System.out.println("Creating StreptococcusPyogenes at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusPyogenes - CellMembrane is complete");
					...
					System.out.println("In StreptococcusPyogenes - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusPyogenes Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusPyogenes Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusPyogenes");
		method.setMethodName("Penetrate StreptococcusPyogenes");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusPyogenes		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusPyogenes : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusPyogenes:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusPyogenes("StreptococcusPyogenes:00001", "StreptococcusPyogenes", 
				//		"StreptococcusPyogenes", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusPyogenes:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusPyogenes("StreptococcusPyogenes:00160", "StreptococcusPyogenes", 
				//	"StreptococcusPyogenes", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusPyogenes NoParent");		
			}

			
			System.out.println("Created StreptococcusPyogenes Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusPyogenes");
			throw new ServerException("Remote Exception StreptococcusPyogenes():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusPyogenes.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusPyogenes
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusPyogenes.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusPyogenes!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusPyogenes - View Internal");
			// We do nada as the StreptococcusPyogenes Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusPyogenes - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusPyogenes: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusPyogenes at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StreptococcusPyogenes.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusPyogenesGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusPyogenes'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusPyogenes - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusPyogenes - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusPyogenes - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					System.out.println("Creating StreptococcusPyogenes at Position: " + 
						...
						for (int k=0; k<4; k++) {
							...
							body+= " <ImageTexture containerField='texture' " +
							    " url='../images/StreptococcusPyogenes.png'/>";  
		...
		//System.out.println("StreptococcusPyogenes X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusPyogenes: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusPyogenes)) {				
	...
	{
		// StreptococcusPyogenes Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusPyogenes Bacteria hemolyzes Red Blood Cells
