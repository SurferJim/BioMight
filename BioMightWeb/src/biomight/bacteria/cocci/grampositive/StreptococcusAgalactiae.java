...
public class StreptococcusAgalactiae extends Bacteria {
	private String bacteriaName = "StreptococcusAgalactiae";
...
private GlutamicAcid glutamicAcid;
	...
	public StreptococcusAgalactiae()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusAgalactiaeRef, null, null);
	}

	public StreptococcusAgalactiae(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusAgalactiae(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusAgalactiae.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusAgalactiae Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusAgalactiae,
			// Go get the details for the current StreptococcusAgalactiae is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusAgalactiae				
				System.out.println("Getting the StreptococcusAgalactiae MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusAgalactiae Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusAgalactiae - Fimbriae is complete");
				...
				System.out.println("In StreptococcusAgalactiae - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StreptococcusAgalactiae, Constants.StreptococcusAgalactiaeRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusAgalactiae directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusAgalactiaeInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusAgalactiae Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusAgalactiae");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusAgalactiae for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusAgalactiae NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusAgalactiae
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusAgalactiae: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusAgalactiae at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusAgalactiae				
					System.out.println("Creating StreptococcusAgalactiae at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusAgalactiae - CellMembrane is complete");
					...
					System.out.println("In StreptococcusAgalactiae - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusAgalactiae Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusAgalactiae Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusAgalactiae");
		method.setMethodName("Penetrate StreptococcusAgalactiae");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusAgalactiae		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusAgalactiae : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusAgalactiae:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusAgalactiae("StreptococcusAgalactiae:00001", "StreptococcusAgalactiae", 
				//		"StreptococcusAgalactiae", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusAgalactiae:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusAgalactiae("StreptococcusAgalactiae:00160", "StreptococcusAgalactiae", 
				//	"StreptococcusAgalactiae", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusAgalactiae NoParent");		
			}

			
			System.out.println("Created StreptococcusAgalactiae Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusAgalactiae");
			throw new ServerException("Remote Exception StreptococcusAgalactiae():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusAgalactiae.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusAgalactiae
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusAgalactiae.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusAgalactiae!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusAgalactiae - View Internal");
			// We do nada as the StreptococcusAgalactiae Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusAgalactiae - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusAgalactiae: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusAgalactiae at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StreptococcusAgalactiae.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusAgalactiaeGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusAgalactiae'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusAgalactiae - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusAgalactiae - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusAgalactiae - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusAgalactiae X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusAgalactiae: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusAgalactiae)) {				
	...
	{
		// StreptococcusAgalactiae Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusAgalactiae Bacteria hemolyzes Red Blood Cells
