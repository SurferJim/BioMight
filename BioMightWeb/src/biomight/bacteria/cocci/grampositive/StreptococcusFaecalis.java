...
public class StreptococcusFaecalis extends Bacteria {
	private String bacteriaName = "StreptococcusFaecalis";
...
private GlutamicAcid glutamicAcid;;
	...
	public StreptococcusFaecalis()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusFaecalisRef, null, null);
	}

	public StreptococcusFaecalis(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusFaecalis(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusFaecalis.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusFaecalis Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusFaecalis,
			// Go get the details for the current StreptococcusFaecalis is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusFaecalis				
				System.out.println("Getting the StreptococcusFaecalis MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusFaecalis Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusFaecalis - Fimbriae is complete");
				...
				System.out.println("In StreptococcusFaecalis - Flagella is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusFaecalis directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusFaecalisInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusFaecalis Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusFaecalis");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusFaecalis for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusFaecalis NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusFaecalis
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusFaecalis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusFaecalis at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusFaecalis				
					System.out.println("Creating StreptococcusFaecalis at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusFaecalis - CellMembrane is complete");
					...
					System.out.println("In StreptococcusFaecalis - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusFaecalis Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusFaecalis Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusFaecalis");
		method.setMethodName("Penetrate StreptococcusFaecalis");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusFaecalis		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusFaecalis : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusFaecalis:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusFaecalis("StreptococcusFaecalis:00001", "StreptococcusFaecalis", 
				//		"StreptococcusFaecalis", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusFaecalis:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusFaecalis("StreptococcusFaecalis:00160", "StreptococcusFaecalis", 
				//	"StreptococcusFaecalis", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusFaecalis NoParent");		
			}

			
			System.out.println("Created StreptococcusFaecalis Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusFaecalis");
			throw new ServerException("Remote Exception StreptococcusFaecalis():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusFaecalis.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusFaecalis
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusFaecalis.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusFaecalis!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusFaecalis - View Internal");
			// We do nada as the StreptococcusFaecalis Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusFaecalis - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusFaecalis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusFaecalis at Position: " + 
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusFaecalisGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusFaecalis'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusFaecalis - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusFaecalis - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusFaecalis - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusFaecalis X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusFaecalis: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusFaecalis)) {				
	...
	{
		// StreptococcusFaecalis Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusFaecalis Bacteria hemolyzes Red Blood Cells
