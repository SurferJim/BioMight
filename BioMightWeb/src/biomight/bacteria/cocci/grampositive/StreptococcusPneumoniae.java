 ...
 * @author SurferJim
 *
 * Representation of StreptococcusPneumoniae
 */

public class StreptococcusPneumoniae extends Bacteria {
	private String bacteriaName = "StreptococcusPneumoniae";
...
private GlutamicAcid glutamicAcid;
	...
	public StreptococcusPneumoniae()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusPneumoniaeRef, null, null);
	}

	public StreptococcusPneumoniae(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusPneumoniae(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusPneumoniae.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusPneumoniae Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusPneumoniae,
			// Go get the details for the current StreptococcusPneumoniae is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusPneumoniae				
				System.out.println("Getting the StreptococcusPneumoniae MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusPneumoniae Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusPneumoniae - Fimbriae is complete");
				...
				System.out.println("In StreptococcusPneumoniae - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StreptococcusPneumoniae, Constants.StreptococcusPneumoniaeRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusPneumoniae directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusPneumoniaeInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusPneumoniae Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusPneumoniae");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusPneumoniae for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusPneumoniae NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusPneumoniae
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusPneumoniae: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusPneumoniae at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusPneumoniae				
					System.out.println("Creating StreptococcusPneumoniae at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusPneumoniae - CellMembrane is complete");
					...
					System.out.println("In StreptococcusPneumoniae - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusPneumoniae Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusPneumoniae Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusPneumoniae");
		method.setMethodName("Penetrate StreptococcusPneumoniae");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusPneumoniae		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusPneumoniae : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusPneumoniae:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusPneumoniae("StreptococcusPneumoniae:00001", "StreptococcusPneumoniae", 
				//		"StreptococcusPneumoniae", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusPneumoniae:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusPneumoniae("StreptococcusPneumoniae:00160", "StreptococcusPneumoniae", 
				//	"StreptococcusPneumoniae", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusPneumoniae NoParent");		
			}

			
			System.out.println("Created StreptococcusPneumoniae Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusPneumoniae");
			throw new ServerException("Remote Exception StreptococcusPneumoniae():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusPneumoniae.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusPneumoniae
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusPneumoniae.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusPneumoniae!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusPneumoniae - View Internal");
			// We do nada as the StreptococcusPneumoniae Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusPneumoniae - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusPneumoniae: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusPneumoniae at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StreptococcusPneumoniae.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusPneumoniaeGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusPneumoniae'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusPneumoniae - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusPneumoniae - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusPneumoniae - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusPneumoniae X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusPneumoniae: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusPneumoniae)) {				
	...
	{
		// StreptococcusPneumoniae Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusPneumoniae Bacteria hemolyzes Red Blood Cells
