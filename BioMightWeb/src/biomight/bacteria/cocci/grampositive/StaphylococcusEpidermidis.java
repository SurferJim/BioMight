...
public class StaphylococcusEpidermidis extends Bacteria {
	private String bacteriaName = "StaphylococcusEpidermidis";
...
private GlutamicAcid glutamicAcid;
	...
	public StaphylococcusEpidermidis()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StaphylococcusEpidermidisRef, null, null);
	}

	public StaphylococcusEpidermidis(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StaphylococcusEpidermidis(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StaphylococcusEpidermidis.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StaphylococcusEpidermidis Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StaphylococcusEpidermidis,
			// Go get the details for the current StaphylococcusEpidermidis is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StaphylococcusEpidermidis				
				System.out.println("Getting the StaphylococcusEpidermidis MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StaphylococcusEpidermidis Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StaphylococcusEpidermidis - Fimbriae is complete");
				...
				System.out.println("In StaphylococcusEpidermidis - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StaphylococcusEpidermidis, Constants.StaphylococcusEpidermidisRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StaphylococcusEpidermidis directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StaphylococcusEpidermidisInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StaphylococcusEpidermidis Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StaphylococcusEpidermidis");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StaphylococcusEpidermidis for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StaphylococcusEpidermidis NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StaphylococcusEpidermidis
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StaphylococcusEpidermidis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StaphylococcusEpidermidis at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StaphylococcusEpidermidis				
					System.out.println("Creating StaphylococcusEpidermidis at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StaphylococcusEpidermidis - CellMembrane is complete");
					...
					System.out.println("In StaphylococcusEpidermidis - Ribosomes are complete");
		...
		System.out.println("CreateStaphylococcusEpidermidis Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StaphylococcusEpidermidis Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StaphylococcusEpidermidis");
		method.setMethodName("Penetrate StaphylococcusEpidermidis");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StaphylococcusEpidermidis		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StaphylococcusEpidermidis : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StaphylococcusEpidermidis:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStaphylococcusEpidermidis("StaphylococcusEpidermidis:00001", "StaphylococcusEpidermidis", 
				//		"StaphylococcusEpidermidis", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StaphylococcusEpidermidis:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStaphylococcusEpidermidis("StaphylococcusEpidermidis:00160", "StaphylococcusEpidermidis", 
				//	"StaphylococcusEpidermidis", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StaphylococcusEpidermidis NoParent");		
			}

			
			System.out.println("Created StaphylococcusEpidermidis Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StaphylococcusEpidermidis");
			throw new ServerException("Remote Exception StaphylococcusEpidermidis():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StaphylococcusEpidermidis.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StaphylococcusEpidermidis
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StaphylococcusEpidermidis.jpg'/>\n" +
		...
		"<Scene>\n" +
		"<WorldInfo\n" +
		"title='StaphylococcusEpidermidis'\n" +
		...
		System.out.println("Getting X3D for StaphylococcusEpidermidis!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StaphylococcusEpidermidis - View Internal");
			// We do nada as the StaphylococcusEpidermidis Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StaphylococcusEpidermidis - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StaphylococcusEpidermidis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StaphylococcusEpidermidis at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StaphylococcusEpidermidis.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StaphylococcusEpidermidisGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StaphylococcusEpidermidis'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StaphylococcusEpidermidis - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StaphylococcusEpidermidis - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StaphylococcusEpidermidis - View Hawkeye  1X");
		...
		//System.out.println("StaphylococcusEpidermidis X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StaphylococcusEpidermidis: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StaphylococcusEpidermidis)) {				
	...
	{
		// StaphylococcusEpidermidis Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StaphylococcusEpidermidis Bacteria hemolyzes Red Blood Cells
