...
public class StaphylococcusSaprophyticus extends Bacteria {
	private String bacteriaName = "StaphylococcusSaprophyticus";
...
private GlutamicAcid glutamicAcid;
	...
	public StaphylococcusSaprophyticus()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StaphylococcusSaprophyticusRef, null, null);
	}

	public StaphylococcusSaprophyticus(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StaphylococcusSaprophyticus(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StaphylococcusSaprophyticus.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StaphylococcusSaprophyticus Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StaphylococcusSaprophyticus,
			// Go get the details for the current StaphylococcusSaprophyticus is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StaphylococcusSaprophyticus				
				System.out.println("Getting the StaphylococcusSaprophyticus MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StaphylococcusSaprophyticus Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StaphylococcusSaprophyticus - Fimbriae is complete");
				...
				System.out.println("In StaphylococcusSaprophyticus - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StaphylococcusSaprophyticus, Constants.StaphylococcusSaprophyticusRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StaphylococcusSaprophyticus directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StaphylococcusSaprophyticusInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StaphylococcusSaprophyticus Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StaphylococcusSaprophyticus");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StaphylococcusSaprophyticus for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StaphylococcusSaprophyticus NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StaphylococcusSaprophyticus
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StaphylococcusSaprophyticus: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StaphylococcusSaprophyticus at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StaphylococcusSaprophyticus				
					System.out.println("Creating StaphylococcusSaprophyticus at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StaphylococcusSaprophyticus - CellMembrane is complete");
					...
					System.out.println("In StaphylococcusSaprophyticus - Ribosomes are complete");
		...
		System.out.println("CreateStaphylococcusSaprophyticus Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StaphylococcusSaprophyticus Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StaphylococcusSaprophyticus");
		method.setMethodName("Penetrate StaphylococcusSaprophyticus");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StaphylococcusSaprophyticus		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StaphylococcusSaprophyticus : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StaphylococcusSaprophyticus:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStaphylococcusSaprophyticus("StaphylococcusSaprophyticus:00001", "StaphylococcusSaprophyticus", 
				//		"StaphylococcusSaprophyticus", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StaphylococcusSaprophyticus:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStaphylococcusSaprophyticus("StaphylococcusSaprophyticus:00160", "StaphylococcusSaprophyticus", 
				//	"StaphylococcusSaprophyticus", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StaphylococcusSaprophyticus NoParent");		
			}

			
			System.out.println("Created StaphylococcusSaprophyticus Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StaphylococcusSaprophyticus");
			throw new ServerException("Remote Exception StaphylococcusSaprophyticus():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StaphylococcusSaprophyticus.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StaphylococcusSaprophyticus
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StaphylococcusSaprophyticus.jpg'/>\n" +
		...
		"<Scene>\n" +
		"<WorldInfo\n" +
		"title='StaphylococcusSaprophyticus'\n" +
		...
		System.out.println("Getting X3D for StaphylococcusSaprophyticus!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StaphylococcusSaprophyticus - View Internal");
			// We do nada as the StaphylococcusSaprophyticus Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StaphylococcusSaprophyticus - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StaphylococcusSaprophyticus: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StaphylococcusSaprophyticus at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StaphylococcusSaprophyticus.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StaphylococcusSaprophyticusGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StaphylococcusSaprophyticus'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StaphylococcusSaprophyticus - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StaphylococcusSaprophyticus - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StaphylococcusSaprophyticus - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					eRing = 2;
					ringSize = 1.5 * radius * eRing;
					double[][] offSets2 = new double[6][3];
					...
					ringSize = 2.25*radius * eRing;
					radiansLatitude =  Math.toRadians(45);
					radiansLongitude =  Math.toRadians(30);
					//System.out.println("radiansLatitude: " + radiansLatitude + " radiansLongitude: " + radiansLongitude);
					...
					offSets2[3][0] = xPos+1.75*radius;
					offSets2[3][1] = yPos ;
					offSets2[3][2] = zPos;
					
					
					ringSize = 3.25*radius * eRing;
					radiansLatitude =  Math.toRadians(45);
					radiansLongitude =  Math.toRadians(45);
					//System.out.println("radiansLatitude: " + radiansLatitude + " radiansLongitude: " + radiansLongitude);
		...
		//System.out.println("StaphylococcusSaprophyticus X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StaphylococcusSaprophyticus: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StaphylococcusSaprophyticus)) {				
	...
	{
		// StaphylococcusSaprophyticus Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StaphylococcusSaprophyticus Bacteria hemolyzes Red Blood Cells
