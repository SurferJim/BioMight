...
public class StreptococcusSalivarius extends Bacteria {
	private String bacteriaName = "StreptococcusSalivarius";
...
private GlutamicAcid glutamicAcid;
	...
	public StreptococcusSalivarius()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusSalivariusRef, null, null);
	}

	public StreptococcusSalivarius(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusSalivarius(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusSalivarius.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusSalivarius Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusSalivarius,
			// Go get the details for the current StreptococcusSalivarius is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusSalivarius				
				System.out.println("Getting the StreptococcusSalivarius MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusSalivarius Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusSalivarius - Fimbriae is complete");
				...
				System.out.println("In StreptococcusSalivarius - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StreptococcusSalivarius, Constants.StreptococcusSalivariusRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusSalivarius directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusSalivariusInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusSalivarius Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusSalivarius");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusSalivarius for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusSalivarius NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusSalivarius
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusSalivarius: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusSalivarius at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusSalivarius				
					System.out.println("Creating StreptococcusSalivarius at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusSalivarius - CellMembrane is complete");
					...
					System.out.println("In StreptococcusSalivarius - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusSalivarius Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusSalivarius Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusSalivarius");
		method.setMethodName("Penetrate StreptococcusSalivarius");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusSalivarius		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusSalivarius : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusSalivarius:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusSalivarius("StreptococcusSalivarius:00001", "StreptococcusSalivarius", 
				//		"StreptococcusSalivarius", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusSalivarius:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusSalivarius("StreptococcusSalivarius:00160", "StreptococcusSalivarius", 
				//	"StreptococcusSalivarius", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusSalivarius NoParent");		
			}

			
			System.out.println("Created StreptococcusSalivarius Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusSalivarius");
			throw new ServerException("Remote Exception StreptococcusSalivarius():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusSalivarius.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusSalivarius
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusSalivarius.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusSalivarius!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusSalivarius - View Internal");
			// We do nada as the StreptococcusSalivarius Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusSalivarius - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusSalivarius: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusSalivarius at Position: " + 
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusSalivariusGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusSalivarius'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusSalivarius - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusSalivarius - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusSalivarius - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusSalivarius X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusSalivarius: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusSalivarius)) {				
	...
	{
		// StreptococcusSalivarius Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusSalivarius Bacteria hemolyzes Red Blood Cells
