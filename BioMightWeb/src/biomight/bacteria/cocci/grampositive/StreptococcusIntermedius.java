...
public class StreptococcusIntermedius extends Bacteria {
	private String bacteriaName = "StreptococcusIntermedius";
...
private GlutamicAcid glutamicAcid;
	...
	public StreptococcusIntermedius()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusIntermediusRef, null, null);
	}

	public StreptococcusIntermedius(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusIntermedius(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusIntermedius.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusIntermedius Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusIntermedius,
			// Go get the details for the current StreptococcusIntermedius is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusIntermedius				
				System.out.println("Getting the StreptococcusIntermedius MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusIntermedius Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusIntermedius - Fimbriae is complete");
				...
				System.out.println("In StreptococcusIntermedius - Flagella is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusIntermedius directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusIntermediusInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusIntermedius Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusIntermedius");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusIntermedius for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusIntermedius NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusIntermedius
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusIntermedius: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusIntermedius at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusIntermedius				
					System.out.println("Creating StreptococcusIntermedius at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusIntermedius - CellMembrane is complete");
					...
					System.out.println("In StreptococcusIntermedius - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusIntermedius Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusIntermedius Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusIntermedius");
		method.setMethodName("Penetrate StreptococcusIntermedius");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusIntermedius		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusIntermedius : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusIntermedius:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusIntermedius("StreptococcusIntermedius:00001", "StreptococcusIntermedius", 
				//		"StreptococcusIntermedius", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusIntermedius:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusIntermedius("StreptococcusIntermedius:00160", "StreptococcusIntermedius", 
				//	"StreptococcusIntermedius", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusIntermedius NoParent");		
			}

			
			System.out.println("Created StreptococcusIntermedius Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusIntermedius");
			throw new ServerException("Remote Exception StreptococcusIntermedius():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusIntermedius.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusIntermedius
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusIntermedius.jpg'/>\n" +
		...
		"<Scene>\n" +
		"<WorldInfo\n" +
		"title='StreptococcusIntermedius'\n" +
		...
		System.out.println("Getting X3D for StreptococcusIntermedius!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusIntermedius - View Internal");
			// We do nada as the StreptococcusIntermedius Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusIntermedius - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusIntermedius: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusIntermedius at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StreptococcusIntermedius.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusIntermediusGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusIntermedius'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusIntermedius - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusIntermedius - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusIntermedius - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusIntermedius X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusIntermedius: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusIntermedius)) {				
	...
	{
		// StreptococcusIntermedius Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusIntermedius Bacteria hemolyzes Red Blood Cells
