...
public class StreptococcusBovis extends Bacteria {
	private String bacteriaName = "StreptococcusBovis";
...
private GlutamicAcid glutamicAcid;
	...
	public StreptococcusBovis()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusBovisRef, null, null);
	}

	public StreptococcusBovis(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public StreptococcusBovis(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusBovis.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusBovis Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusBovis,
			// Go get the details for the current StreptococcusBovis is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusBovis				
				System.out.println("Getting the StreptococcusBovis MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusBovis Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusBovis - Fimbriae is complete");
				...
				System.out.println("In StreptococcusBovis - Flagella is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusBovis directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusBovisInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusBovis Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusBovis");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusBovis for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusBovis NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusBovis
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusBovis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusBovis at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusBovis				
					System.out.println("Creating StreptococcusBovis at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusBovis - CellMembrane is complete");
					...
					System.out.println("In StreptococcusBovis - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusBovis Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusBovis Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusBovis");
		method.setMethodName("Penetrate StreptococcusBovis");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusBovis		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusBovis : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusBovis:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusBovis("StreptococcusBovis:00001", "StreptococcusBovis", 
				//		"StreptococcusBovis", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusBovis:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusBovis("StreptococcusBovis:00160", "StreptococcusBovis", 
				//	"StreptococcusBovis", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusBovis NoParent");		
			}

			
			System.out.println("Created StreptococcusBovis Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusBovis");
			throw new ServerException("Remote Exception StreptococcusBovis():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusBovis.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusBovis
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusBovis.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusBovis!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusBovis - View Internal");
			// We do nada as the StreptococcusBovis Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusBovis - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusBovis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusBovis at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/StreptococcusBovis.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusBovisGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusBovis'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusBovis - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusBovis - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusBovis - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusBovis X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusBovis: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusBovis)) {				
	...
	{
		// StreptococcusBovis Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusBovis Bacteria hemolyzes Red Blood Cells
