...
public class BordetellaPertussis extends Bacteria {
	private String bacteriaName = "StreptococcusMitis";
...
private GlutamicAcid glutamicAcid;;
	...
	public BordetellaPertussis()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.StreptococcusMitisRef, null, null);
	}

	public BordetellaPertussis(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public BordetellaPertussis(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/StreptococcusMitis.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In StreptococcusMitis Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of StreptococcusMitis,
			// Go get the details for the current StreptococcusMitis is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the StreptococcusMitis				
				System.out.println("Getting the StreptococcusMitis MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("StreptococcusMitis Instance created from SubComponents : " + parentID);
				...
				System.out.println("In StreptococcusMitis - Fimbriae is complete");
				...
				System.out.println("In StreptococcusMitis - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.StreptococcusMitis, Constants.StreptococcusMitisRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a StreptococcusMitis directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye StreptococcusMitisInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have StreptococcusMitis Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - StreptococcusMitis");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the StreptococcusMitis for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("StreptococcusMitis NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the StreptococcusMitis
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating StreptococcusMitis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating StreptococcusMitis at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the StreptococcusMitis				
					System.out.println("Creating StreptococcusMitis at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In StreptococcusMitis - CellMembrane is complete");
					...
					System.out.println("In StreptococcusMitis - Ribosomes are complete");
		...
		System.out.println("CreateStreptococcusMitis Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING StreptococcusMitis Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate StreptococcusMitis");
		method.setMethodName("Penetrate StreptococcusMitis");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the StreptococcusMitis		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the StreptococcusMitis : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("StreptococcusMitis:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateStreptococcusMitis("StreptococcusMitis:00001", "StreptococcusMitis", 
				//		"StreptococcusMitis", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("StreptococcusMitis:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateStreptococcusMitis("StreptococcusMitis:00160", "StreptococcusMitis", 
				//	"StreptococcusMitis", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate StreptococcusMitis NoParent");		
			}

			
			System.out.println("Created StreptococcusMitis Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - StreptococcusMitis");
			throw new ServerException("Remote Exception StreptococcusMitis():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the StreptococcusMitis.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the StreptococcusMitis
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='StreptococcusMitis.jpg'/>\n" +
		...
		System.out.println("Getting X3D for StreptococcusMitis!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D StreptococcusMitis - View Internal");
			// We do nada as the StreptococcusMitis Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusMitis - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating StreptococcusMitis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating StreptococcusMitis at Position: " + 
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='StreptococcusMitisGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='StreptococcusMitis'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D StreptococcusMitis - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D StreptococcusMitis - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D StreptococcusMitis - View Hawkeye  1X");
		...
		//System.out.println("StreptococcusMitis X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for StreptococcusMitis: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.StreptococcusMitis)) {				
	...
	{
		// StreptococcusMitis Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// StreptococcusMitis Bacteria hemolyzes Red Blood Cells
