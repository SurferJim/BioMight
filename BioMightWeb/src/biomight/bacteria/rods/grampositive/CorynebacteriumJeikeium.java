...
public class CorynebacteriumJeikeium extends Bacteria {
	private String bacteriaName = "CorynebacteriumJeikeium";
	...
	public CorynebacteriumJeikeium()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.CorynebacteriumJeikeiumRef, null, null);
	}

	public CorynebacteriumJeikeium(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public CorynebacteriumJeikeium(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/CorynebacteriumJeikeium.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In CorynebacteriumJeikeium Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of CorynebacteriumJeikeium,
			// Go get the details for the current CorynebacteriumJeikeium is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the CorynebacteriumJeikeium				
				System.out.println("Getting the CorynebacteriumJeikeium MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("CorynebacteriumJeikeium Instance created from SubComponents : " + parentID);
				...
				System.out.println("In CorynebacteriumJeikeium - Fimbriae is complete");
				...
				System.out.println("In CorynebacteriumJeikeium - Flagella is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a CorynebacteriumJeikeium directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye CorynebacteriumJeikeiumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have CorynebacteriumJeikeium Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - CorynebacteriumJeikeium");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the CorynebacteriumJeikeium for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("CorynebacteriumJeikeium NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating CorynebacteriumJeikeium at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the CorynebacteriumJeikeium				
					System.out.println("Creating CorynebacteriumJeikeium at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In CorynebacteriumJeikeium - CellMembrane is complete");
					...
					System.out.println("In CorynebacteriumJeikeium - Ribosomes are complete");
		...
		System.out.println("CreateCorynebacteriumJeikeium Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING CorynebacteriumJeikeium Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate CorynebacteriumJeikeium");
		method.setMethodName("Penetrate CorynebacteriumJeikeium");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the CorynebacteriumJeikeium : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("CorynebacteriumJeikeium:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateCorynebacteriumJeikeium("CorynebacteriumJeikeium:00001", "CorynebacteriumJeikeium", 
				//		"CorynebacteriumJeikeium", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("CorynebacteriumJeikeium:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateCorynebacteriumJeikeium("CorynebacteriumJeikeium:00160", "CorynebacteriumJeikeium", 
				//	"CorynebacteriumJeikeium", componentID, parentID, currentPoints);
			}
			else if (parentID.equals("")) 
			{	
				System.out.println("Calling Generate CorynebacteriumJeikeium NoParent");		
			}
			...
			System.out.println("Exception Getting Components - CorynebacteriumJeikeium");
			throw new ServerException("Remote Exception CorynebacteriumJeikeium():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the CorynebacteriumJeikeium.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the CorynebacteriumJeikeium
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='CorynebacteriumJeikeium.jpg'/>\n" +
		...
		System.out.println("Getting X3D for CorynebacteriumJeikeium!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D CorynebacteriumJeikeium - View Internal");
			// We do nada as the CorynebacteriumJeikeium Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D CorynebacteriumJeikeium - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating CorynebacteriumJeikeium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating CorynebacteriumJeikeium at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/CorynebacteriumJeikeium.jpg'/>";
		...
		{
			System.out.println("Get X3D CorynebacteriumJeikeium - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D CorynebacteriumJeikeium - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D CorynebacteriumJeikeium - View Hawkeye  1X");
		...
		//System.out.println("CorynebacteriumJeikeium X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for CorynebacteriumJeikeium: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.CorynebacteriumJeikeium)) {				
	...
	{
		// CorynebacteriumJeikeium Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// CorynebacteriumJeikeium Bacteria hemolyzes Red Blood Cells
