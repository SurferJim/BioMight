...
import biomight.chemistry.molecule.neurotoxin.*;
...
public class ClostridiumTetani extends Bacteria {
	private String bacteriaName = "ClostridiumTetani";
	...
	public ClostridiumTetani()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.ClostridiumTetaniRef, null, null);
	}

	public ClostridiumTetani(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public ClostridiumTetani(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In ClostridiumTetani Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of ClostridiumTetani,
			// Go get the details for the current ClostridiumTetani is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the ClostridiumTetani				
				System.out.println("Getting the ClostridiumTetani MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);		
				System.out.println("ClostridiumTetani Instance created from SubComponents : " + parentID);
			
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.ClostridiumTetani, Constants.ClostridiumTetaniRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a ClostridiumTetani directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye ClostridiumTetaniInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have ClostridiumTetani Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the ClostridiumTetani for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("ClostridiumTetani NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the ClostridiumTetani
				...
				{
					System.out.println("Creating ClostridiumTetani at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the ClostridiumTetani				
					System.out.println("Creating ClostridiumTetani at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In ClostridiumTetani - CellMembrane is complete");
					...
					System.out.println("In ClostridiumTetani - Fimbriae is complete");
					...
					System.out.println("In ClostridiumTetani - Flagella is complete");
					...
					System.out.println("In ClostridiumTetani - Ribosomes are complete");
					*/
		...
		System.out.println("CreateClostridiumTetani Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate ClostridiumTetani");
		method.setMethodName("Penetrate ClostridiumTetani");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the ClostridiumTetani : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateClostridiumTetani("ClostridiumTetani:00001", "ClostridiumTetani", 
			//		"ClostridiumTetani", componentID, parentID, currentPoints);			
			
			System.out.println("Created ClostridiumTetani Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - ClostridiumTetani");
			throw new ServerException("Remote Exception ClostridiumTetani():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the ClostridiumTetani.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the ClostridiumTetani
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='ClostridiumTetani.jpg'/>\n" +
		...
		System.out.println("Getting X3D for ClostridiumTetani!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D ClostridiumTetani - View Internal");
			// We do nada as the ClostridiumTetani Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D ClostridiumTetani - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating ClostridiumTetani: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating ClostridiumTetani at Position: " + 
		...
		{
			System.out.println("Get X3D ClostridiumTetani - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D ClostridiumTetani - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += cellMembrane.getX3D(true)  
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D ClostridiumTetani - View Hawkeye  1X");
	...
	{
		// ClostridiumTetani Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// ClostridiumTetani Bacteria hemolyzes Red Blood Cells
