...
public class VibrioCholerae extends Bacteria {
	private String bacteriaName = "VibrioCholerae";
...
private GlutamicAcid glutamicAcid;
	...
	public VibrioCholerae()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.VibrioCholeraeRef, null, null);
	}

	public VibrioCholerae(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public VibrioCholerae(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In VibrioCholerae Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of VibrioCholerae,
			// Go get the details for the current VibrioCholerae is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the VibrioCholerae				
				System.out.println("Getting the VibrioCholerae MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);		
				System.out.println("VibrioCholerae Instance created from SubComponents : " + parentID);
			
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.VibrioCholerae, Constants.VibrioCholeraeRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a VibrioCholerae directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye VibrioCholeraeInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have VibrioCholerae Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the VibrioCholerae for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("VibrioCholerae NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the VibrioCholerae
				...
				{
					System.out.println("Creating VibrioCholerae at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the VibrioCholerae				
					System.out.println("Creating VibrioCholerae at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In VibrioCholerae - CellMembrane is complete");
					...
					System.out.println("In VibrioCholerae - Fimbriae is complete");
					...
					System.out.println("In VibrioCholerae - Flagella is complete");
					...
					System.out.println("In VibrioCholerae - Ribosomes are complete");
					*/
		...
		System.out.println("CreateVibrioCholerae Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate VibrioCholerae");
		method.setMethodName("Penetrate VibrioCholerae");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the VibrioCholerae : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateVibrioCholerae("VibrioCholerae:00001", "VibrioCholerae", 
			//		"VibrioCholerae", componentID, parentID, currentPoints);			
			
			System.out.println("Created VibrioCholerae Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - VibrioCholerae");
			throw new ServerException("Remote Exception VibrioCholerae():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the VibrioCholerae.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the VibrioCholerae
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='VibrioCholerae.jpg'/>\n" +
		...
		System.out.println("Getting X3D for VibrioCholerae!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D VibrioCholerae - View Internal");
			// We do nada as the VibrioCholerae Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D VibrioCholerae - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating VibrioCholerae: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating VibrioCholerae at Position: " + 
		...
		{
			System.out.println("Get X3D VibrioCholerae - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D VibrioCholerae - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += cellMembrane.getX3D(true)  
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D VibrioCholerae - View Hawkeye  1X");
	...
	{
		// VibrioCholerae Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// VibrioCholerae Bacteria hemolyzes Red Blood Cells
