 ...
 * @author SurferJim
 *
 * Representation of SalmonellaTyphis
 ********************************************************************************/

public class SalmonellaTyphisOrig extends BioMightBase {
private ArrayList salmonellaTyphis;

	
	/********************************************************************************************************************
	 *  SalmonellaTyphi
	 * 
	 * This method will instantiate the SalmonellaTyphi that are defined for the current model.  
	 * 
	 ********************************************************************************************************************/

	public SalmonellaTyphisOrig()
	{		
		int localVP= Constants.BIO_CHILD;
		int localLOD = Constants.VIEW_HAWKEYE;
		create(localVP, localLOD, Constants.SalmonellaTyphisRef, null, null);
	}
	...
	public SalmonellaTyphisOrig(String parentID)
	{				
	...
	public SalmonellaTyphisOrig(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	...
	{
		this.setImage("images/SalmonellaTyphi.jpg");
		
		salmonellaTyphis = new ArrayList();
		...
		String bioTemplate="SalmonellaTyphi.x3d";
	
		if (bioMightMethods != null){
			System.out.println("EXECUTING SalmonellaTyphi Methods: " + bioMightMethods.size());
			executeMethods(bioMightMethods);
		}
		
		// Get the information from the database via the Enterprise Bean.  This will get
		// a collection of SalmonellaTyphi
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Getting SalmonellaTyphi Transform: " + componentID + "   " +  parentID);
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			bioMightTransforms = bioMightBean.getComponents(Constants.SalmonellaTyphiRef, parentID);   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - SalmonellaTyphi");
			throw new ServerException("Remote Exception getComponents():", e); 	
		}
		
		
		// Run through the collection of salmonellaTyphis and build them into the model
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Have SalmonellaTyphi NumTransforms: " + transforms.size());
		...
		{
			// Get the information for the salmonellaTyphis we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Creating SalmonellaTyphi: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			
			// Create an instance of a SalmonellaTyphi for each tranform specified
			localVP = Constants.VIEW_INTERNAL;
			localLOD = Constants.MAG1X;
			
			SalmonellaTyphi salmonellaTyphi = new SalmonellaTyphi(localVP, localLOD, bioMightTransform.getId(), bioMightProperties, bioMightMethods);		
			System.out.println("SalmonellaTyphi Created!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			salmonellaTyphis.add(salmonellaTyphi);
			System.out.println("Added salmonellaTyphi to Collection!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());


			String propDesc = Constants.SalmonellaTyphiRef + "-GramNegative, Enteric Bacillus, motile, FaculativeAnaerobe";
			initProperty(bioMightTransform.getName(), Constants.SalmonellaTyphi, Constants.SalmonellaTyphiRef, propDesc,  bioMightTransform.getId(), bioPos, bioScale, bioOrient, Constants.SINGLE_COMPONENT, bioTemplate, false);
		...
		// Set up methods that will be available to the salmonellaTyphis
	...
	public void initMethods(ArrayList<BioMightMethodView> bioMightMethodsIn) {
		...
		if (bioMightMethodsIn.size() == 0) {
			System.out.println("New Set of SalmonellaTyphiMethods: " + bioMightMethodsIn.size());
			BioMightMethodView method = new BioMightMethodView();
			method.setCanonicalName(Constants.SalmonellaTyphi);
		...
		{
			System.out.println("Using Existing SalmonellaTyphiMethods: " + bioMightMethodsIn.size());
	...
	public String getX3D(boolean snipet) {
		...
		{
			System.out.println("Assembling X3D for SalmonellaTyhpis HawkEyeView - size: " + salmonellaTyphis.size());
			
			// Run through the collection of SalmonellaTyhpis and assemble the X3D for each
			for (int i=0; i<salmonellaTyphis.size(); i++)
			{
				// Get the information for the salmonellaTyphis
				//SalmonellaTyhpis salmonellaTyphis = (SalmonellaTyhpis) salmonellaTyphis.get(i);
				//System.out.println("Getting X3D for SalmonellaTyhpis: " + salmonellaTyphis.getComponentID());
				//body += salmonellaTyphis.getX3D(true);
			}		
			...
			for (int i=0; i<transforms.size(); i++)
			{
				...
				for (int cells=0; cells<1; cells++)
				{
					...
					body += "<Transform DEF='TRANSFORM_BACTI_TOP" + cells + "'\n";
					...
					body +=  "scale='" 	+ 1.0 + " "
					 	...
					 	"<Shape DEF='START_SPHERE_" + bioMightTransform.getId() + "_" + cells + "' \n" +
					...
					body+= " <Material DEF='Rust'\n" +
						...
						"\n<TouchSensor DEF='TSENSOR_" + bioMightTransform.getId() + "' \n" +
		                   " description='Salmonella Typhi Cell Membrane'\n" +
			               " containerField='children'/> \n" +
					...
					//**********************************************************
					body += "<Transform DEF='TRANSFORM_ENDSPHERE'\n";
					...
					body +=  "scale='" 	+ 1.0 + " "
					 	...
					 	"<Shape DEF='SHAPE_END" + bioMightTransform.getId() + "' \n" +
					    " containerField='children'>\n" +
					    " <Appearance\n" +
					    "  containerField='appearance'>\n";
					...
					for (int bodySegment=0; bodySegment<0; bodySegment++)
					//for (int bodySegment=0; bodySegment<numSegments; bodySegment++)
					{
						...
						for (int longitude=0; longitude<numLongitude; longitude++) 
						{
							...
							bioGroup = "FlagellaGroup" + "L" + longitude + "B" + bodySegment;
							...
							///*****************************************
							body += "<Transform DEF='TRANSFORM_BaseSpike" + "L" + longitude * bodySegment + longitude + "'\n";
						 	body += "translation='" 
						 		+ xPos + " " 
			 					+ yPos + " "
			 					+ zPos + "'\n";					
							...
							body +=  "scale='1.0 1.0 1.0'>\n" +
							 	...
							 	"<Shape DEF='SHAPE_" + bioMightTransform.getId() + (longitude * bodySegment) + longitude + "'\n" +
							    " containerField='children'>\n" +
							    " <Appearance\n" +
							    "  containerField='appearance'>\n";
							...
							for (int v=0; v<randoLength; v++) {
								...
								//*****************************************************
								body += "<Transform DEF='TRANSFORM_STARTSPHERE_" + (longitude * bodySegment) + longitude + "'\n";
								...
								// Create the Cylinder to represent Cell Membrane
								//**************************************************
								body += "<Transform DEF='TRANSFORM_FLAGELLA" +  (longitude * bodySegment) + longitude + "'\n";
						...
						***/
						...
						//body += "\n\n</Transform>\n\n";
							...
							for (int v=0; v<30; v++) {
								...
								// Create little sphere connector where the position is
								//*****************************************************
								body += "<Transform DEF='TRANSFORM_STARTSPHERE" + bioGroup + "'\n";
		...
		{	
			System.out.println("Assembling X3D for SalmonellaTyhpis InternalView - size: " + salmonellaTyphis.size());
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("SalmonellaTyphi-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for SalmonellaTyphi: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.SalmonellaTyphi)) {				
			...
			if (fired)
			{	
				System.out.println("SalmonellaTyphi - Methods have fired.   Calling SalmonellaTyphi Save method!");
			}
		}
	}
	 ...
	 * SET COLONY SIZE 
	 *
	 * This method will set the Colony size for the SalmonellaTyphi.  
	 *****************************************************************************************/
	public void setColonySize(int size) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("SalmonellaTyphi-SetColony Size: " + size);
		
		// Generate the SalmonellaTyphi		
		BioMightCellularBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the SalmonellaTyphi Colony: " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			//bioMightBean = (BioMightCellularBeanLocal) ctx.lookup("biomight.ejb.BioMightCellularBeanLocal");
			bioMightBean = (BioMightCellularBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightCellularBean!biomight.ejb.BioMightCellularBeanLocal");
	
			double currentPoints[][] = null;
		//	int success = bioMightBean.generateSalmonellaTyphi(size, "SalmonellaTyphi:00001", "SalmonellaTyphi", 
		//		"SalmonellaTyphi", this.componentID, this.parentID, currentPoints);
			
			System.out.println("Created SalmonellaTyphi Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Creating Colony - SalmonellaTyphi");
			throw new ServerException("Remote Exception SalmonellaTyphiEpithelium():", e); 	
		}
	}
