...
public class SalmonellaTyphi extends Bacteria {
	private String bacteriaName = "SalmonellaTyphi";
...
private GlutamicAcid glutamicAcid;;
	...
	public SalmonellaTyphi()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.SalmonellaTyphiRef, null, null);
	}

	public SalmonellaTyphi(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public SalmonellaTyphi(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In SalmonellaTyphi Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of SalmonellaTyphi,
			// Go get the details for the current SalmonellaTyphi is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the SalmonellaTyphi				
				System.out.println("Getting the SalmonellaTyphi MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("SalmonellaTyphi Instance created from SubComponents : " + parentID);
				...
				System.out.println("In SalmonellaTyphi - Fimbriae is complete");
				...
				System.out.println("In SalmonellaTyphi - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.SalmonellaTyphi, Constants.SalmonellaTyphiRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a SalmonellaTyphi directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye SalmonellaTyphiInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have SalmonellaTyphi Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the SalmonellaTyphi for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("SalmonellaTyphi NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the SalmonellaTyphi
				...
				{
					System.out.println("Creating SalmonellaTyphi at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the SalmonellaTyphi				
					System.out.println("Creating SalmonellaTyphi at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In SalmonellaTyphi - CellMembrane is complete");
					...
					System.out.println("In SalmonellaTyphi - Ribosomes are complete");
		...
		System.out.println("CreateSalmonellaTyphi Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate SalmonellaTyphi");
		method.setMethodName("Penetrate SalmonellaTyphi");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the SalmonellaTyphi : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("SalmonellaTyphi:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateSalmonellaTyphi("SalmonellaTyphi:00001", "SalmonellaTyphi", 
				//		"SalmonellaTyphi", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("SalmonellaTyphi:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateSalmonellaTyphi("SalmonellaTyphi:00160", "SalmonellaTyphi", 
				//	"SalmonellaTyphi", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate SalmonellaTyphi NoParent");		
			}
			...
			System.out.println("Exception Getting Components - SalmonellaTyphi");
			throw new ServerException("Remote Exception SalmonellaTyphi():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the SalmonellaTyphi.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the SalmonellaTyphi
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='SalmonellaTyphi.jpg'/>\n" +
		...
		System.out.println("Getting X3D for SalmonellaTyphi!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D SalmonellaTyphi - View Internal");
			// We do nada as the SalmonellaTyphi Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D SalmonellaTyphi - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating SalmonellaTyphi: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating SalmonellaTyphi at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/SalmonellaTyphi.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='SalmonellaTyphiGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='SalmonellaTyphi'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D SalmonellaTyphi - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D SalmonellaTyphi - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D SalmonellaTyphi - View Hawkeye  1X");
		...
		//System.out.println("SalmonellaTyphi X3D: " + body);		
	...
	{
		// SalmonellaTyphi Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// SalmonellaTyphi Bacteria hemolyzes Red Blood Cells
