...
public class SalmonellaEnterica extends Bacteria {
	private String bacteriaName = "SalmonellaEnterica";
...
private GlutamicAcid glutamicAcid;;
	...
	public SalmonellaEnterica()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.SalmonellaEntericaRef, null, null);
	}

	public SalmonellaEnterica(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public SalmonellaEnterica(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In SalmonellaEnterica Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of SalmonellaEnterica,
			// Go get the details for the current SalmonellaEnterica is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the SalmonellaEnterica				
				System.out.println("Getting the SalmonellaEnterica MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("SalmonellaEnterica Instance created from SubComponents : " + parentID);
				...
				System.out.println("In SalmonellaEnterica - Fimbriae is complete");
				...
				System.out.println("In SalmonellaEnterica - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.SalmonellaEnterica, Constants.SalmonellaEntericaRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a SalmonellaEnterica directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye SalmonellaEntericaInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have SalmonellaEnterica Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the SalmonellaEnterica for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("SalmonellaEnterica NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the SalmonellaEnterica
				...
				{
					System.out.println("Creating SalmonellaEnterica at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the SalmonellaEnterica				
					System.out.println("Creating SalmonellaEnterica at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In SalmonellaEnterica - CellMembrane is complete");
					...
					System.out.println("In SalmonellaEnterica - Ribosomes are complete");
		...
		System.out.println("CreateSalmonellaEnterica Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate SalmonellaEnterica");
		method.setMethodName("Penetrate SalmonellaEnterica");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the SalmonellaEnterica : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("SalmonellaEnterica:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateSalmonellaEnterica("SalmonellaEnterica:00001", "SalmonellaEnterica", 
				//		"SalmonellaEnterica", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("SalmonellaEnterica:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateSalmonellaEnterica("SalmonellaEnterica:00160", "SalmonellaEnterica", 
				//	"SalmonellaEnterica", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate SalmonellaEnterica NoParent");		
			}
			...
			System.out.println("Exception Getting Components - SalmonellaEnterica");
			throw new ServerException("Remote Exception SalmonellaEnterica():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the SalmonellaEnterica.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the SalmonellaEnterica
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='SalmonellaEnterica.jpg'/>\n" +
		...
		System.out.println("Getting X3D for SalmonellaEnterica!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D SalmonellaEnterica - View Internal");
			// We do nada as the SalmonellaEnterica Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D SalmonellaEnterica - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating SalmonellaEnterica: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating SalmonellaEnterica at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/SalmonellaEnterica.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='SalmonellaEntericaGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='SalmonellaEnterica'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D SalmonellaEnterica - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D SalmonellaEnterica - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D SalmonellaEnterica - View Hawkeye  1X");
		...
		//System.out.println("SalmonellaEnterica X3D: " + body);		
	...
	{
		// SalmonellaEnterica Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// SalmonellaEnterica Bacteria hemolyzes Red Blood Cells
