 ...
 * @author SurferJim
 *
 * Representation of SalmonellaEntericas
 ********************************************************************************/

public class SalmonellaEntericas extends BioMightBase {
private ArrayList salmonellaEntericas;

	
	/********************************************************************************************************************
	 *  SalmonellaEnterica
	 * 
	 * This method will instantiate the SalmonellaEnterica that are defined for the current model.  
	 * 
	 ********************************************************************************************************************/

	public SalmonellaEntericas()
	{		
		int localVP= Constants.BIO_CHILD;
		int localLOD = Constants.VIEW_HAWKEYE;
		create(localVP, localLOD, Constants.SalmonellaEntericasRef, null, null);
	}
	...
	public SalmonellaEntericas(String parentID)
	{				
	...
	public SalmonellaEntericas(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	...
	{
		this.setImage("images/SalmonellaEnterica.jpg");
		
		salmonellaEntericas = new ArrayList();
		...
		String bioTemplate="SalmonellaEnterica.x3d";
	
		if (bioMightMethods != null){
			System.out.println("EXECUTING SalmonellaEnterica Methods: " + bioMightMethods.size());
			executeMethods(bioMightMethods);
		}
		
		// Get the information from the database via the Enterprise Bean.  This will get
		// a collection of SalmonellaEnterica
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Getting SalmonellaEnterica Transform: " + componentID + "   " +  parentID);
			InitialContext ctx = new InitialContext();
			BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
			bioMightTransforms = bioMightBean.getComponents(Constants.SalmonellaEntericaRef, parentID);   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - SalmonellaEnterica");
			throw new ServerException("Remote Exception getComponents():", e); 	
		}
		
		
		// Run through the collection of salmonellaEntericas and build them into the model
		ArrayList transforms = bioMightTransforms.getTransforms();
		System.out.println("Have SalmonellaEnterica NumTransforms: " + transforms.size());
		...
		{
			// Get the information for the salmonellaEntericas we are creating
			BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
			System.out.println("Creating SalmonellaEnterica: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			
			// Create an instance of a SalmonellaEnterica for each tranform specified
			localVP = Constants.VIEW_INTERNAL;
			localLOD = Constants.MAG1X;
			
			SalmonellaEnterica salmonellaEnterica = new SalmonellaEnterica(localVP, localLOD, bioMightTransform.getId(), bioMightProperties, bioMightMethods);		
			System.out.println("SalmonellaEnterica Created!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
			salmonellaEntericas.add(salmonellaEnterica);
			System.out.println("Added salmonellaEnterica to Collection!: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());


			String propDesc = Constants.SalmonellaEntericaRef + "-GramNegative, Enteric Bacillus, motile, FaculativeAnaerobe";
			initProperty(bioMightTransform.getName(), Constants.SalmonellaEnterica, Constants.SalmonellaEntericaRef, propDesc,  bioMightTransform.getId(), bioPos, bioScale, bioOrient, Constants.SINGLE_COMPONENT, bioTemplate, false);
		...
		// Set up methods that will be available to the salmonellaEntericas
	...
	public void initMethods(ArrayList<BioMightMethodView> bioMightMethodsIn) {
		...
		if (bioMightMethodsIn.size() == 0) {
			System.out.println("New Set of SalmonellaEntericaMethods: " + bioMightMethodsIn.size());
			BioMightMethodView method = new BioMightMethodView();
			method.setCanonicalName(Constants.SalmonellaEnterica);
		...
		{
			System.out.println("Using Existing SalmonellaEntericaMethods: " + bioMightMethodsIn.size());
	...
	public String getX3D(boolean snipet) {
		...
		{
			System.out.println("Assembling X3D for SalmonellaTyhpis HawkEyeView - size: " + salmonellaEntericas.size());
			
			// Run through the collection of SalmonellaTyhpis and assemble the X3D for each
			for (int i=0; i<salmonellaEntericas.size(); i++)
			{
				// Get the information for the salmonellaEntericas
				//SalmonellaTyhpis salmonellaEntericas = (SalmonellaTyhpis) salmonellaEntericas.get(i);
				//System.out.println("Getting X3D for SalmonellaTyhpis: " + salmonellaEntericas.getComponentID());
				//body += salmonellaEntericas.getX3D(true);
			}		
			...
			for (int i=0; i<transforms.size(); i++)
			{
				...
				for (int cells=0; cells<1; cells++)
				{
					...
					body+= " <ImageTexture containerField='texture' " +
					    " url='../images/SalmonellaEntericas.png'/>";
					...
					body+= " <Material DEF='Rust'\n" +
					   ...
					   // "transparency='" 		+ bioMightTransform.getMaterial().getTransparency() + "'\n" +
						...
						"\n<TouchSensor DEF='TSENSOR_" + bioMightTransform.getId() + "' \n" +
		                   " description='SalmonellaEntericas'\n" +
			               " containerField='children'/> \n" +
					...
					body+= " <Material DEF='Rust'\n" +
					    ...
					    "diffuseColor='" + 
					 	...
					 	"</Appearance>\n" +
					 	"<Cylinder DEF='SalmonellaEntericasGeoSphere'\n" +
					 	"containerField='geometry'\n" +
					 	"top='false'\n" +
					 	"radius='" + radius +" '\n" +
					 	"height='" + height +"'/>\n" +
						...
						"\n<TouchSensor DEF='TSENSOR_" + bioMightTransform.getId() + "' \n" +
		                   " description='Salmonella Enterica Cell Membrane'\n" +
			               " containerField='children'/> \n" +
		...
		{	
			System.out.println("Assembling X3D for SalmonellaTyhpis InternalView - size: " + salmonellaEntericas.size());
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("SalmonellaEnterica-Executing Methods: " + bioMightMethods.size());
		for (int j=0; j<bioMightMethods.size(); j++){
			
			// Get the parameter from the list and if it is not
			// empty execute the associated method using it
			BioMightMethodView bioMightMethod = (BioMightMethodView) bioMightMethods.get(j);
			System.out.println("Have BioMightMethod for SalmonellaEnterica: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.SalmonellaEnterica)) {				
			...
			if (fired)
			{	
				System.out.println("SalmonellaEnterica - Methods have fired.   Calling SalmonellaEnterica Save method!");
			}
		}
	}
	 ...
	 * SET COLONY SIZE 
	 *
	 * This method will set the Colony size for the SalmonellaEnterica.  
	 *****************************************************************************************/
	public void setColonySize(int size) {
		...
		// associated methods
		boolean fired = false;
		System.out.println("SalmonellaEnterica-SetColony Size: " + size);
		
		// Generate the SalmonellaEnterica		
		BioMightCellularBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the SalmonellaEnterica Colony: " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			//bioMightBean = (BioMightCellularBeanLocal) ctx.lookup("biomight.ejb.BioMightCellularBeanLocal");
			bioMightBean = (BioMightCellularBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightCellularBean!biomight.ejb.BioMightCellularBeanLocal");
	
			double currentPoints[][] = null;
		//	int success = bioMightBean.generateSalmonellaEnterica(size, "SalmonellaEnterica:00001", "SalmonellaEnterica", 
		//		"SalmonellaEnterica", this.componentID, this.parentID, currentPoints);
			
			System.out.println("Created SalmonellaEnterica Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Creating Colony - SalmonellaEnterica");
			throw new ServerException("Remote Exception SalmonellaEntericaEpithelium():", e); 	
		}
	}
