...
public class AeromonasHydrophilia extends Bacteria {
	private String bacteriaName = "AeromonasHydrophilia";
	...
	public AeromonasHydrophilia()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.AeromonasHydrophiliaRef, null, null);
	}

	public AeromonasHydrophilia(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public AeromonasHydrophilia(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In AeromonasHydrophilia Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of AeromonasHydrophilia,
			// Go get the details for the current AeromonasHydrophilia is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the AeromonasHydrophilia				
				System.out.println("Getting the AeromonasHydrophilia MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("AeromonasHydrophilia Instance created from SubComponents : " + parentID);
				...
				System.out.println("In AeromonasHydrophilia - Fimbriae is complete");
				...
				System.out.println("In AeromonasHydrophilia - Flagella is complete");
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a AeromonasHydrophilia directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye AeromonasHydrophiliaInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have AeromonasHydrophilia Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the AeromonasHydrophilia for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("AeromonasHydrophilia NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the AeromonasHydrophilia
				...
				{
					System.out.println("Creating AeromonasHydrophilia at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the AeromonasHydrophilia				
					System.out.println("Creating AeromonasHydrophilia at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In AeromonasHydrophilia - CellMembrane is complete");
					...
					System.out.println("In AeromonasHydrophilia - Ribosomes are complete");
		...
		System.out.println("CreateAeromonasHydrophilia Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate AeromonasHydrophilia");
		method.setMethodName("Penetrate AeromonasHydrophilia");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the AeromonasHydrophilia : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("AeromonasHydrophilia:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateAeromonasHydrophilia("AeromonasHydrophilia:00001", "AeromonasHydrophilia", 
				//		"AeromonasHydrophilia", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("AeromonasHydrophilia:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateAeromonasHydrophilia("AeromonasHydrophilia:00160", "AeromonasHydrophilia", 
				//	"AeromonasHydrophilia", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate AeromonasHydrophilia NoParent");		
			}
			...
			System.out.println("Exception Getting Components - AeromonasHydrophilia");
			throw new ServerException("Remote Exception AeromonasHydrophilia():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the AeromonasHydrophilia.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the AeromonasHydrophilia
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='AeromonasHydrophilia.jpg'/>\n" +
		...
		System.out.println("Getting X3D for AeromonasHydrophilia!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D AeromonasHydrophilia - View Internal");
			// We do nada as the AeromonasHydrophilia Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D AeromonasHydrophilia - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating AeromonasHydrophilia: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating AeromonasHydrophilia at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/AeromonasHydrophilia.jpg'/>";
		...
		{
			System.out.println("Get X3D AeromonasHydrophilia - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D AeromonasHydrophilia - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D AeromonasHydrophilia - View Hawkeye  1X");
	...
	{
		// AeromonasHydrophilia Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// AeromonasHydrophilia Bacteria hemolyzes Red Blood Cells
