...
public class SalmonellaTyphimurium extends Bacteria {
	private String bacteriaName = "SalmonellaTyphimurium";
...
private GlutamicAcid glutamicAcid;;
	...
	public SalmonellaTyphimurium()
	{
		...
		create(localVP, localLOD, Constants.SalmonellaTyphimuriumRef, null, null);
	}

	public SalmonellaTyphimurium(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public SalmonellaTyphimurium(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/SalmonellaTyphimurium.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In SalmonellaTyphimurium Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of SalmonellaTyphimurium,
			// Go get the details for the current SalmonellaTyphimurium is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the SalmonellaTyphimurium				
				System.out.println("Getting the SalmonellaTyphimurium MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("SalmonellaTyphimurium Instance created from SubComponents : " + parentID);
				...
				System.out.println("In SalmonellaTyphimurium - Fimbriae is complete");
				...
				System.out.println("In SalmonellaTyphimurium - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.SalmonellaTyphimurium, Constants.SalmonellaTyphimuriumRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a SalmonellaTyphimurium directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye SalmonellaTyphimuriumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have SalmonellaTyphimurium Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - SalmonellaTyphimurium");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the SalmonellaTyphimurium for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("SalmonellaTyphimurium NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the SalmonellaTyphimurium
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating SalmonellaTyphimurium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				this.componentID = bioMightTransform.getId();

		
				if (localLOD == Constants.MAG1X)
				{
					System.out.println("Creating SalmonellaTyphimurium at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the SalmonellaTyphimurium				
					System.out.println("Creating SalmonellaTyphimurium at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In SalmonellaTyphimurium - CellMembrane is complete");
					...
					System.out.println("In SalmonellaTyphimurium - Ribosomes are complete");
		...
		System.out.println("CreateSalmonellaTyphimurium Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING SalmonellaTyphimurium Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate SalmonellaTyphimurium");
		method.setMethodName("Penetrate SalmonellaTyphimurium");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	{
		// Generate the SalmonellaTyphimurium		
		BioMightBeanLocal bioMightBean;
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the SalmonellaTyphimurium : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("SalmonellaTyphimurium:01")) {
				
				// Generate the Palm
				double[] startPos = {3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
						
				//int success = bioMightBean.generateSalmonellaTyphimurium("SalmonellaTyphimurium:00001", "SalmonellaTyphimurium", 
				//		"SalmonellaTyphimurium", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("SalmonellaTyphimurium:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				double[][] currentPoints = BioGraphics.octogonXPlane(startPos, circumference);
				
				//int success = bioMightBean.generateSalmonellaTyphimurium("SalmonellaTyphimurium:00160", "SalmonellaTyphimurium", 
				//	"SalmonellaTyphimurium", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate SalmonellaTyphimurium NoParent");		
			}

			
			System.out.println("Created SalmonellaTyphimurium Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - SalmonellaTyphimurium");
			throw new ServerException("Remote Exception SalmonellaTyphimurium():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the SalmonellaTyphimurium.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the SalmonellaTyphimurium
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='SalmonellaTyphimurium.jpg'/>\n" +
		...
		System.out.println("Getting X3D for SalmonellaTyphimurium!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D SalmonellaTyphimurium - View Internal");
			// We do nada as the SalmonellaTyphimurium Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D SalmonellaTyphimurium - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating SalmonellaTyphimurium: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating SalmonellaTyphimurium at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/SalmonellaTyphimurium.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='SalmonellaTyphimuriumGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='SalmonellaTyphimurium'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D SalmonellaTyphimurium - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D SalmonellaTyphimurium - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D SalmonellaTyphimurium - View Hawkeye  1X");
		...
		//System.out.println("SalmonellaTyphimurium X3D: " + body);		
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for SalmonellaTyphimurium: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.SalmonellaTyphimurium)) {				
	...
	{
		// SalmonellaTyphimurium Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// SalmonellaTyphimurium Bacteria hemolyzes Red Blood Cells
