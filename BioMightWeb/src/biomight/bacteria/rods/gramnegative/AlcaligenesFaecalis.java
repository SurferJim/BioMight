...
public class AlcaligenesFaecalis extends Bacteria {
	private String bacteriaName = "AlcaligenesFaecalis";
	...
	public AlcaligenesFaecalis()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.AlcaligenesFaecalisRef, null, null);
	}

	public AlcaligenesFaecalis(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public AlcaligenesFaecalis(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In AlcaligenesFaecalis Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of AlcaligenesFaecalis,
			// Go get the details for the current AlcaligenesFaecalis is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the AlcaligenesFaecalis				
				System.out.println("Getting the AlcaligenesFaecalis MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
				System.out.println("AlcaligenesFaecalis Instance created from SubComponents : " + parentID);
				...
				System.out.println("In AlcaligenesFaecalis - Fimbriae is complete");
				...
				System.out.println("In AlcaligenesFaecalis - Flagella is complete");
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.AlcaligenesFaecalis, Constants.AlcaligenesFaecalisRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a AlcaligenesFaecalis directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye AlcaligenesFaecalisInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have AlcaligenesFaecalis Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - AlcaligenesFaecalis");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of AlcaligenesFaecalis and build them into the model
			// In the default case, we get one instance of the AlcaligenesFaecalis for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("AlcaligenesFaecalis NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the AlcaligenesFaecalis
				...
				{
					System.out.println("Creating AlcaligenesFaecalis at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the AlcaligenesFaecalis				
					System.out.println("Creating AlcaligenesFaecalis at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In AlcaligenesFaecalis - CellMembrane is complete");
					...
					System.out.println("In AlcaligenesFaecalis - Ribosomes are complete");
		...
		System.out.println("CreateAlcaligenesFaecalis Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate AlcaligenesFaecalis");
		method.setMethodName("Penetrate AlcaligenesFaecalis");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the AlcaligenesFaecalis : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("AlcaligenesFaecalis:01")) {
				...
				String startID = "";
				//int success = bioMightBean.generateAlcaligenesFaecalis(startID, "AlcaligenesFaecalis", 
				//		"AlcaligenesFaecalis", componentID, parentID, currentPoints);			
				
			}
			else if (parentID.equals("")) 
			{	
				System.out.println("Calling Generate AlcaligenesFaecalis NoParent");		
			}
			...
			System.out.println("Exception Getting Components - AlcaligenesFaecalis");
			throw new ServerException("Remote Exception AlcaligenesFaecalis():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the AlcaligenesFaecalis.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the AlcaligenesFaecalis
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='AlcaligenesFaecalis.jpg'/>\n" +
		...
		System.out.println("Getting X3D for AlcaligenesFaecalis!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D AlcaligenesFaecalis - View Internal");
			// We do nada as the AlcaligenesFaecalis Data is retrieved in the collection object
			// and the X3D is generated there
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D AlcaligenesFaecalis - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating AlcaligenesFaecalis: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating AlcaligenesFaecalis at Position: " + 
				...
				body+= " <ImageTexture containerField='texture' " +
				    " url='../images/AlcaligenesFaecalis.jpg'/>";
				...
				body+= " <Material DEF='Rust'\n" +
				    ...
				    "diffuseColor='" + 
				 	...
				 	"</Appearance>\n" +
				 	"<Sphere DEF='AlcaligenesFaecalisGeoSphere'\n" +
				 	"containerField='geometry'\n" +
				 	"radius='" + bioMightTransform.getRadius() +"'/>\n" +
				 	"</Shape>\n" +
				 	
					"\n<TouchSensor DEF='TSENSOR_"+ bioMightTransform.getId()+ "' \n" +
	                   " description='AlcaligenesFaecalis'\n" +
		               " containerField='children'/> \n" +
		...
		{
			System.out.println("Get X3D AlcaligenesFaecalis - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D AlcaligenesFaecalis - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D AlcaligenesFaecalis - View Hawkeye  1X");
				...
				for (int i=0; i<transforms.size(); i++)
				{
					...
					for (int k=0; k<12; k++) {
						...
						body+= " <ImageTexture containerField='texture' " +
						    " url='../images/AlcaligenesFaecalis.png'/>";
						...
						body+= " <Material DEF='Rust'\n" +
						    ...
						    "diffuseColor='" + 
						 	    ...
						 	    bioMightTransform.getMaterial().getDiffuseColor().getBlue() + "'/>\n" +
						 	"</Appearance>\n" +
						 	"<Cylinder DEF='AlcaligenesFaecalisGeoSphere'\n" +
						 	"containerField='geometry'\n" +
						 	"radius='" + bioMightTransform.getRadius() +" '\n" +
		...
		//System.out.println("AlcaligenesFaecalis X3D: " + body);		
	...
	{
		// AlcaligenesFaecalis Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// AlcaligenesFaecalis Bacteria hemolyzes Red Blood Cells
