...
public class TreponemaPallidum extends Bacteria {
	private String bacteriaName = "TreponemaPallidum";
...
private GlutamicAcid glutamicAcid;
	...
	public TreponemaPallidum()
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
		create(localVP, localLOD, Constants.TreponemaPallidumRef, null, null);
	}

	public TreponemaPallidum(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public TreponemaPallidum(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In TreponemaPallidum Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of TreponemaPallidum,
			// Go get the details for the current TreponemaPallidum is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the TreponemaPallidum				
				System.out.println("Getting the TreponemaPallidum MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);		
				System.out.println("TreponemaPallidum Instance created from SubComponents : " + parentID);
			
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.TreponemaPallidum, Constants.TreponemaPallidumRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a TreponemaPallidum directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye TreponemaPallidumInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have TreponemaPallidum Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the TreponemaPallidum for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("TreponemaPallidum NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the TreponemaPallidum
				...
				{
					System.out.println("Creating TreponemaPallidum at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the TreponemaPallidum				
					System.out.println("Creating TreponemaPallidum at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In TreponemaPallidum - CellMembrane is complete");
					...
					System.out.println("In TreponemaPallidum - Fimbriae is complete");
					...
					System.out.println("In TreponemaPallidum - Flagella is complete");
					...
					System.out.println("In TreponemaPallidum - Ribosomes are complete");
					*/
		...
		System.out.println("CreateTreponemaPallidum Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate TreponemaPallidum");
		method.setMethodName("Penetrate TreponemaPallidum");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the TreponemaPallidum : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateTreponemaPallidum("TreponemaPallidum:00001", "TreponemaPallidum", 
			//		"TreponemaPallidum", componentID, parentID, currentPoints);			
			
			System.out.println("Created TreponemaPallidum Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - TreponemaPallidum");
			throw new ServerException("Remote Exception TreponemaPallidum():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the TreponemaPallidum.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the TreponemaPallidum
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='TreponemaPallidum.jpg'/>\n" +
		...
		System.out.println("Getting X3D for TreponemaPallidum!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D TreponemaPallidum - View Internal");
			// We do nada as the TreponemaPallidum Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D TreponemaPallidum - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating TreponemaPallidum: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating TreponemaPallidum at Position: " + 
		...
		{
			System.out.println("Get X3D TreponemaPallidum - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D TreponemaPallidum - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += cellMembrane.getX3D(true)  
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D TreponemaPallidum - View Hawkeye  1X");
	...
	{
		// TreponemaPallidum Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// TreponemaPallidum Bacteria hemolyzes Red Blood Cells
