 ...
 * @author SurferJim
 *
 * Representation of a Ribosome
 * 
 **************************************************************************************/
public class Ribosome extends BioMightBase{
	...
	public Ribosome()
	{		
		// Create the Base Ribosome
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, Constants.RibosomeRef, null, null);
	}
	
	public Ribosome(String parentID)
	{				
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
	...
	public Ribosome(String parentID, BioMightPosition bioMightPosition)
	{
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, parentID, null, null);
	...
	// the Ribosomes parent object and we do not have  to go to the database 
	// to get additional informaion
	public Ribosome(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	 ...
	 * CREATE
	 * 
	 * This method will create the Ribosome
	 * 
	 ********************************************************************************************************************/

	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
			this.setImage("images/Ribosome.jpg");
			...
			if (localVP == Constants.VIEW_INTERNAL)			
			{
				...
				System.out.println("In Ribosome Create() - Already Set: " + parentID);				

				// We already have the data for the current instance of Ribosome,
				// Go get the details for the current Ribosome is LOD is set
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the Ribosome				
					System.out.println("Getting the Ribosome Details: " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					//cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightMethods);									
					System.out.println("Ribosome Instance created from SubComponents : " + parentID);
				
					//System.out.println("In Ribosome - Creating Endosomes");
			...
			{
				// This is when one is accessing a Ribosome directly
				// Get the information from the database via the Enterprise Bean		
				try {
					// Get the information from the database via the Enterprise Bean		
					System.out.println("Getting HawkEye RibosomeInfo for ParentID: " + parentID);
					InitialContext ctx = new InitialContext();
					BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
					bioMightTransforms = bioMightBean.getComponent(parentID);
					System.out.println("Have Ribosome Info from EJB");   	
				}catch (Exception e) { 
					System.out.println("Exception Getting Components - Ribosome");
					throw new ServerException("Remote Exception getComponents():", e); 	
				}
					
				// Run through the collection of Ribosomes and build them into the model
				// In the default case, we get one instance of the Ribosome for each eye
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("Ribosome NumTransforms: " + transforms.size());
				for (int i=0; i<transforms.size(); i++)
				{
					...
					{
						// Go get the finer details of the Ribosome				
						System.out.println("Creating Ribosome : " + parentID + " lod: " + localLOD);
						localVP = Constants.VIEW_HAWKEYE; 
						localLOD = Constants.MAG1X;

						System.out.println("Creating RibosomeMembrane : " + parentID + " lod: " + localLOD);
						...
						System.out.println("In Ribosome - RibosomeMembrane is complete");
			...
			System.out.println("CreateRibosome Completed");
			...
			if (bioMightMethods != null){
				System.out.println("EXECUTING Ribosome Methods: " + bioMightMethods.size());
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Ribosome : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
	
			double circumference = 0.125;
			
			if (componentID.equals("Ribosome:01")) {
				...
				//int success = bioMightBean.generateRibosome("Ribosome:00001", "Ribosome", 
				//		"Ribosome", componentID, parentID, currentPoints);			
				
			}
			else if (componentID.equals("Ribosome:02"))
			{
				// Generate the Elbow
				double[] startPos = {-3.75,-23.0,-6.0};
				...
				//int success = bioMightBean.generateRibosome("Ribosome:00160", "Ribosome", 
				//	"Ribosome", componentID, parentID, currentPoints);
			...
			{	
				System.out.println("Calling Generate Ribosome NoParent");		
			}
			...
			System.out.println("Exception Getting Components - Ribosome");
			throw new ServerException("Remote Exception Ribosome():", e); 	
		}
	}
	...
	public void executeMethods(ArrayList<String> methodParams) {
		...
		// associated methods
		System.out.println("Ribosome Executing Methods");
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Ribosome.  It runs through each of its components and collects up their
	 * representations and then assembles them into one unified X3D model.
	 * 
	 ********************************************************************************************************************/
	public String getX3D(boolean snipet) {
		
		// Assembe the Ribosome
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		"<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.0//EN\" \"http://www.web3d.org/specifications/x3d-3.0.dtd\">\n " +
		"<X3D profile='Immersive' >\n" +
		"<head>\n" +
		 "<meta name='BioMightImage' content='Ribosome.jpg'/>\n" +
		...
		{			
			// We do nada as the Ribosome Data is retrieved in the collection object
		...
		else if (viewPerspective == Constants.VIEW_HAWKEYE) 
		{
			...
			for (int i=0; i<transforms.size(); i++)
			{
				...
				BioMightTransform bioMightTransform = (BioMightTransform) transforms.get(i);
				System.out.println("Creating Ribosome: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Ribosome at Position: " + 
				...
				if (lod == Constants.MAG1X)		
				{
					...
					body+= " <Material DEF='Rust'\n" +
					    ...
					    "diffuseColor='" + 
					 	...
					 	"</Appearance>\n" +
					 	"<Cylinder DEF='RibosomeGeoSphere'\n" +
					 	...
					 	/**
						"<Cylinder DEF='RibosomeGeoCylinder'\n" +
						 "containerField='geometry'\n" +
		...
		body+= "<Viewpoint DEF='Viewpoint_Ribosome'\n" +
				 "description='Viewpoint_Ribosome'\n" +
				 "jump='true'\n" +
				 "fieldOfView='0.785'\n" +
				 "position='0.0 0.0 3.0'\n" +
				 "orientation='0 0 1 0'/>\n";
		
		//System.out.println("Ribosome X3D: " + body);		
