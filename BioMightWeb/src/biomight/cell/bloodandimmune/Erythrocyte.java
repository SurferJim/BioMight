...
public class Erythrocyte extends BioMightBase {
	
	private String bacteriaName = "Erythrocyte";
	...
	public Erythrocyte()
	{
		...
		create(localVP, localLOD, Constants.ErythrocyteRef, null, null);
	}

	public Erythrocyte(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Erythrocyte(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Erythrocyte Create() - ViewInternal - Already Set: " + parentID);				

			
			// We already have the data for the current instance of Erythrocyte,
			// Go get the details for the current Erythrocyte is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Erythrocyte				
				System.out.println("Getting the Erythrocyte MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);		
				System.out.println("Erythrocyte Instance created from SubComponents : " + parentID);
			
	
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.Erythrocyte, Constants.ErythrocyteRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Erythrocyte directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye ErythrocyteInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Erythrocyte Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the Erythrocyte for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Erythrocyte NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the Erythrocyte
				...
				{
					System.out.println("Creating Erythrocyte at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the Erythrocyte				
					System.out.println("Creating Erythrocyte at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In Erythrocyte - CellMembrane is complete");
					...
					System.out.println("In Erythrocyte - Fimbriae is complete");
					...
					System.out.println("In Erythrocyte - Flagella is complete");
					...
					System.out.println("In Erythrocyte - Ribosomes are complete");
					*/
		...
		System.out.println("CreateErythrocyte Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Erythrocyte");
		method.setMethodName("Penetrate Erythrocyte");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Erythrocyte : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateErythrocyte("Erythrocyte:00001", "Erythrocyte", 
			//		"Erythrocyte", componentID, parentID, currentPoints);			
			
			System.out.println("Created Erythrocyte Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - Erythrocyte");
			throw new ServerException("Remote Exception Erythrocyte():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Erythrocyte.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Erythrocyte.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Erythrocyte!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Erythrocyte - View Internal");
			// We do nada as the Erythrocyte Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Erythrocyte - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Erythrocyte: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Erythrocyte at Position: " + 
		...
		{
			System.out.println("Get X3D Erythrocyte - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Erythrocyte - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += cellMembrane.getX3D(true);
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Erythrocyte - View Hawkeye  1X");
	...
	{
		// Erythrocyte Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// Erythrocyte Bacteria hemolyzes Red Blood Cells
