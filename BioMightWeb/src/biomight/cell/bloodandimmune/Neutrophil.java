 ...
 * @author SurferJim
 *
 * Implementation of a Neutrophil Cell 
 * 
 ********************************************************************************/

public class Neutrophil extends BioMightBase {
	
	private String bacteriaName = "Neutrophil";
	...
	public Neutrophil()
	{
		...
		create(localVP, localLOD, Constants.NeutrophilRef, null, null);
	}

	public Neutrophil(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Neutrophil(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	{
		this.setImage("images/Neutrophil.jpg");
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Neutrophil Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Neutrophil,
			// Go get the details for the current Neutrophil is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Neutrophil				
				System.out.println("Getting the Neutrophil MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);		
				System.out.println("Neutrophil Instance created from SubComponents : " + parentID);
				
				System.out.println("In Neutrophil - Creating Granules");
				...
				System.out.println("In Neutrophil - Granules are complete");
				
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.Neutrophil, Constants.NeutrophilRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Neutrophil directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye NeutrophilInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Neutrophil Info from EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - Neutrophil");
				throw new ServerException("Remote Exception getComponents():", e); 	
			}
				
			// Run through the collection of BacilliAnthracis and build them into the model
			// In the default case, we get one instance of the Neutrophil for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Neutrophil NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				...
				{
					System.out.println("Creating Neutrophil at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the Neutrophil				
					System.out.println("Creating Neutrophil at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In Neutrophil - CellMembrane is complete");
		
					
					System.out.println("In Neutrophil - HawkEye - Creating Granules");
					granules = new Granules(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);			
					initProperty("Granules", Constants.Granules, Constants.GranuleRef, componentID);
					System.out.println("In Neutrophil - Granules are complete");
					...
					System.out.println("In Neutrophil - Fimbriae is complete");
					...
					System.out.println("In Neutrophil - Flagella is complete");
					...
					System.out.println("In Neutrophil - Ribosomes are complete");
					*/
		...
		System.out.println("CreateNeutrophil Completed");
		...
		if (bioMightMethods != null){
			System.out.println("EXECUTING Neutrophil Methods: " + bioMightMethods.size());
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Neutrophil");
		method.setMethodName("Penetrate Neutrophil");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Neutrophil : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateNeutrophil("Neutrophil:00001", "Neutrophil", 
			//		"Neutrophil", componentID, parentID, currentPoints);			
			
			System.out.println("Created Neutrophil Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - Neutrophil");
			throw new ServerException("Remote Exception Neutrophil():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Neutrophil.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Neutrophil.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Neutrophil!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Neutrophil - View Internal");
			// We do nada as the Neutrophil Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Neutrophil - View Internal - 2X");
			...
			{			
				...
				/*
				System.out.println("Creating Neutrophil: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Neutrophil at Position: " + 
		...
		{
			System.out.println("Get X3D Neutrophil - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Neutrophil - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += cellMembrane.getX3D(true);
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Neutrophil - View Hawkeye  1X");
	...
	public void executeMethods(ArrayList<BioMightMethodView> bioMightMethods) {
		...
		for (int j=0; j<bioMightMethods.size(); j++){
			...
			System.out.println("Have BioMightMethod for Neutrophil: " + bioMightMethod.getMethodName());	
			...
			if (canonicalName.equals(Constants.Neutrophil)) {				
	...
	{
		// Neutrophil Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// Neutrophil Bacteria hemolyzes Red Blood Cells
		if (obj instanceof Neutrophil)
		{
		}
