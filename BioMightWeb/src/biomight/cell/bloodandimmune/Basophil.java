...
public class Basophil extends BioMightBase {
	
	private String bacteriaName = "Basophil";
	...
	public Basophil()
	{
		...
		create(localVP, localLOD, Constants.BasophilRef, null, null);
	}

	public Basophil(String parentID)
	{
		int localVP= Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG1X;
	...
	public Basophil(int hierarchy, int LOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
		...
		if (localVP == Constants.VIEW_INTERNAL)			
		{
			...
			System.out.println("In Basophil Create() - ViewInternal - Already Set: " + parentID);				

			// We already have the data for the current instance of Basophil,
			// Go get the details for the current Basophil is LOD is set
			if (localLOD == Constants.MAG2X)
			{
				// Go get the finer details of the Basophil				
				System.out.println("Getting the Basophil MAG2X : " + parentID);
				localVP = Constants.VIEW_INTERNAL; 
				localLOD = Constants.MAG1X;
				cellMembrane = new CellMembrane(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);		
				System.out.println("Basophil Instance created from SubComponents : " + parentID);
				
				System.out.println("In Basophil - Creating Granules");
				granules = new Granules(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);			
				initProperty("Granules", Constants.Granules, Constants.GranuleRef, componentID);
				System.out.println("In Basophil - Granules are complete");
				
				// initialize the Properties
				//initProperty(bioMightTransform.getName(), Constants.Basophil, Constants.BasophilRef, bioMightTransform.getId());
			}
		...
		else if (localVP == Constants.VIEW_HAWKEYE)
		{
			...
			// This is when one is accessing a Basophil directly
			// Get the information from the database via the Enterprise Bean		
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Getting HawkEye BasophilInfo for ParentID: " + parentID);
				InitialContext ctx = new InitialContext();
				BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
				bioMightTransforms = bioMightBean.getComponent(parentID);
				
				System.out.println("Have Basophil Info from EJB");   	
			}catch (Exception e) { 
			...
			// In the default case, we get one instance of the Basophil for each eye
			ArrayList transforms = bioMightTransforms.getTransforms();
			System.out.println("Basophil NumTransforms: " + transforms.size());
			for (int i=0; i<transforms.size(); i++)
			{
				// Get the information for the Basophil
				...
				{
					System.out.println("Creating Basophil at HawkEye MAG1X - Just Init Properties: " + parentID);
					// initialize the Properties
					initProperties();
				}
				if (localLOD == Constants.MAG2X)
				{
					// Go get the finer details of the Basophil				
					System.out.println("Creating Basophil at HawkEye MAG2X : " + parentID);
					localVP = Constants.VIEW_INTERNAL; 
					localLOD = Constants.MAG1X;
					...
					System.out.println("In Basophil - CellMembrane is complete");
		
					
					System.out.println("In Basophil - HawkEye - Creating Granules");
					granules = new Granules(localVP, localLOD, componentID, bioMightProperties, bioMightMethods);			
					initProperty("Granules", Constants.Granules, Constants.GranuleRef, componentID);
					System.out.println("In Basophil - Granules are complete");
					...
					System.out.println("In Basophil - Fimbriae is complete");
					...
					System.out.println("In Basophil - Flagella is complete");
					...
					System.out.println("In Basophil - Ribosomes are complete");
					*/
		...
		System.out.println("CreateBasophil Completed");
	...
	public void initMethods() {
		...
		method = new BioMightMethodView();
		method.setDisplayName("Penetrate Basophil");
		method.setMethodName("Penetrate Basophil");
		method.setHtmlType("checkbox");
		method.setDataType("boolean");
		methods.add(method);	
	...
	public void generate(String parentID, String componentID)
	{
		...
		try {
			// Get the information from the database via the Enterprise Bean		
			System.out.println("Generating the Basophil : " + componentID + "    " + parentID);
			InitialContext ctx = new InitialContext();
			...
			//int success = bioMightBean.generateBasophil("Basophil:00001", "Basophil", 
			//		"Basophil", componentID, parentID, currentPoints);			
			
			System.out.println("Created Basophil Info using EJB");   	
		}catch (Exception e) { 
			System.out.println("Exception Getting Components - Basophil");
			throw new ServerException("Remote Exception Basophil():", e); 	
		}
	}
	 ...
	 * GET X3D
	 * 
	 * This method will return the X3D for the Basophil.  It runs through each of its components and collects up their
	...
	public String getX3D(boolean snipet) {
		...
		"<head>\n" +
		 "<meta name='BioMightImage' content='Basophil.jpg'/>\n" +
		...
		System.out.println("Getting X3D for Basophil!!!!!!!!");
		...
		{	
			...
			System.out.println("Get X3D Basophil - View Internal");
			// We do nada as the Basophil Data is retrieved in the collection object
			// and the X3D is generated there
			lod = Constants.MAG2X;
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Basophil - View Internal - 2X");
				...
				System.out.println("BioMight Transform extracted radius: " + nucleurRadius);
				
			    ranges = new ArrayList();
				bioRange = new BioMightRange(25, 3, 5, 11, 14, 0,  maxWidth, 0, maxHeight);
				ranges.add(bioRange);
			    
			  	body += BioWebX3DUtils.generateBasophil(bioMightTransform,  bioMightPosition, bioMightOrientation); 	
			...
			{			
				...
				/*
				System.out.println("Creating Basophil: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
				System.out.println("Creating Basophil at Position: " + 
		...
		{
			System.out.println("Get X3D Basophil - View Hawkeye");
			...
			if (lod == Constants.MAG2X)		
			{
				System.out.println("Get X3D Basophil - View Hawkeye -- 2X");
				//We are going to get the X3D from the aggregation objects
				body += cellMembrane.getX3D(true);
			...
			{			
				// We went to the database to get data.  There will be 1 Transform record
				System.out.println("Get X3D Basophil - View Hawkeye  1X");
	...
	{
		// Basophil Bacteria ferments Mannitol
		if (obj instanceof Mannitol)
		{
		}

		// Basophil Bacteria hemolyzes Red Blood Cells
		if (obj instanceof Basophil)
		{
		}
