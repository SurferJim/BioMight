 ...
 * @author SurferJim
 *
 * Representation of a Antibody.  It is composed of proteins.
 * 
 ************************************************************************************/
...
public class Antibody extends BioMightBase {
	...
	public Antibody()
	{		
		// Create the Base Antibody
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, Constants.AntibodyRef, null, null);
	}
	
	public Antibody(String parentID)
	{				
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
	...
	public Antibody(String parentID, BioMightPosition bioMightPosition)
	{
		int localVP =  Constants.VIEW_HAWKEYE;
		int localLOD = Constants.MAG2X;
		create(localVP, localLOD, parentID, null, null);
	...
	public Antibody(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{		
	...
	public void create(int localVP, int localLOD, String parentID, ArrayList<BioMightPropertyView> bioMightProperties, ArrayList<BioMightMethodView> bioMightMethods)
	{
			...
			if (localVP == Constants.VIEW_INTERNAL)			
			{
				...
				System.out.println("In Antibody Create() - Already Set: " + parentID);				
			}
			...
			{	
				...
				// Generate the Antibody Epihelium if needed 
				boolean bGenerate = false;
				if (bGenerate) {
					generate(parentID, componentID);
				}
			
				System.out.println("Creating Antibody Epithelium: " + parentID);				
				cellMembrane = new CellMembrane(parentID, bioMightMethods);
								
				System.out.println("Antibody Instance created from SubComponents : " + parentID);
			}	
			else if (localVP == Constants.VIEW_HAWKEYE)
			{
				// This is when one is accessing a Antibody directly
				// Get the information from the database via the Enterprise Bean		
				try {
					// Get the information from the database via the Enterprise Bean		
					System.out.println("Getting HawkEye AntibodyInfo for ParentID: " + parentID);
					InitialContext ctx = new InitialContext();
					BioMightBeanLocal bioMightBean = (BioMightBeanLocal) ctx.lookup(Constants.BioMightBeanRef);
					bioMightTransforms = bioMightBean.getComponent(parentID);
					System.out.println("Have Antibody Info from EJB");   	
				}catch (Exception e) { 
					System.out.println("Exception Getting Components - Antibody");
					throw new ServerException("Remote Exception getComponents():", e); 	
				}
					
				// Run through the collection of Antibodies and build them into the model
				// In the default case, we get one instance of the Antibody for each eye
				ArrayList transforms = bioMightTransforms.getTransforms();
				System.out.println("Antibody NumTransforms: " + transforms.size());
				for (int i=0; i<transforms.size(); i++)
				{
					...
					//System.out.println("In Antibody - Creating Ribosomes");
					...
					//System.out.println("In Antibody - Granules are complete");
			...
			System.out.println("CreateAntibody Completed");
			...
			this.parentID = parentID;
			if (bioMightMethods != null){
				System.out.println("EXECUTING Antibody Methods: " + bioMightMethods.size());
	...
	private void initProperties() {
		...
		public void generate(String parentID, String componentID)
		{
			...
			try {
				// Get the information from the database via the Enterprise Bean		
				System.out.println("Generating the Antibody Epithelium: " + componentID + "    " + parentID);
				InitialContext ctx = new InitialContext();
				bioMightBean = (BioMightBeanLocal) ctx.lookup("java:global/BioMightWeb/BioMightBean!biomight.ejb.BioMightBeanLocal");
		
				double circumference = 0.125;
				
				if (componentID.equals("Antibody:01")) {
					...
					//int success = bioMightBean.generateAntibody("AntibodyEpithelium:00001", "AntibodyEpithelium", 
					//		"AntibodyEpithelium", componentID, parentID, currentPoints);			
					
				}
				else if (componentID.equals("Antibody:02"))
				{
					// Generate the Elbow
					double[] startPos = {-3.75,-23.0,-6.0};
					...
					//int success = bioMightBean.generateAntibody("Antibody:00160", "Antibody", 
					//	"AntibodyE", componentID, parentID, currentPoints);
			
					
				}
				else if (parentID.equals("")) 
				{	
					System.out.println("Calling Generate AntibodyEpithelium NoParent");		
				}

				
				System.out.println("Created AntibodyEpithelium Info using EJB");   	
			}catch (Exception e) { 
				System.out.println("Exception Getting Components - AntibodyEpithelium");
		...
		public void executeMethods(ArrayList<String> methodParams) {
			...
			// associated methods
			System.out.println("Antibody Executing Methods");
		 ...
		 * GET X3D
		 * 
		 * This method will return the X3D for the Antibody.  It runs through each of its components and collects up their
		...
		public String getX3D(boolean snipet) {
			...
			"<head>\n" +
			 "<meta name='BioMightImage' content='Antibody.jpg'/>\n" +
			...
			System.out.println("Getting X3D for Antibody!!!!!!!!");
			...
			{	
				...
				System.out.println("Get X3D Antibody - View Internal");
				// We do nada as the Antibody Data is retrieved in the collection object
				// and the X3D is generated there
				lod = Constants.MAG2X;
				if (lod == Constants.MAG2X)		
				{
					System.out.println("Get X3D Antibody - View Internal - 2X");
					...
					String componentTypeOut = "AntibodyGlobal";
					body += "<Transform onmouseover=\"showComponent('Antibody');\" DEF='" + componentTypeOut + "'\n";
					body+= "rotation='" 	
							+ "0.0 , " + " "
							+ "0.0 , " + " "
							+ "1.0 , " + " "
							+  randomRotateAngle + "'>\n";
				  	body += BioWebX3DUtils.generateAntibody(bioMightTransform,  bioMightPosition, bioMightOrientation); 	
				  	body += "</Transform>\n";
				}
				...
				{			
					...
					/*
					System.out.println("Creating Antibody: " + bioMightTransform.getName() + "  " + bioMightTransform.getId());
					System.out.println("Creating Antibody at Position: " + 
			...
			{
				System.out.println("Get X3D Antibody - View Hawkeye");
				...
				if (lod == Constants.MAG2X)		
				{
					System.out.println("Get X3D Antibody - View Hawkeye -- 2X");
				...
				{			
					// We went to the database to get data.  There will be 1 Transform record
					System.out.println("Get X3D Antibody - View Hawkeye  1X");
			...
			//System.out.println("Antibody X3D: " + body);		
